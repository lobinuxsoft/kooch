...
| ~~#826~~ | ~~sample the tile's lights, 15 → 2-4~~ | **Built twice.** A light is picked in proportion to what it contributes and divided by the probability of the pick, so two lights land 5 % from the full walk's brightness where two *truncated* ones land 83 % away. `KOOCH_LIGHT_SAMPLES`, capped at 8. The first version chose **per pixel** and the device refused it; the second chooses **per froxel, cooperatively** — see below. It also found what the froxel flicker really was. ✅ **The seed now carries the frame — but only when the temporal resolve is on**, and that condition is the finding. #826 wanted the frame index because without it every frame draws the same sample and the TAA averages an unchanging sequence; `light_sampling`'s own `the_same_view_samples_the_same_lights` wanted the opposite, having *measured* that a frame counter puts the froxel shimmer back (7075 pixels moving between two identical frames). Both hold, in different regimes: noise that changes every frame is what a resolve integrates and what an unresolved image shows as shimmer. So `IntiFrame::frame_index` is the counter with the resolve on and `0` with it off — a project without TAA renders bit-for-bit what it did before. Two tests, one per regime |
