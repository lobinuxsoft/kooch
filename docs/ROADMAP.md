...
🔴 **#1118 — in an element mode a click can never select another entity.** Found in the
smoke test. The handler returns before `entity_at` is reached, and the comment
defending that `return` is mine: it was correct when Face was the only element mode,
because a ray hits a face or it does not. Vertex and edge pick within **12 pixels**, so
most clicks miss the element while landing on geometry, and the guard now fires on
exactly the case it was written to exclude. **The rule was not wrong — its premise
stopped being true and nothing re-checked it.**

Then #1106 (cheapest, shortens a real blockout most), #1086 triplanar **before** #1069's
