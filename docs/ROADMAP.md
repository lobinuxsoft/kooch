...
Last updated 2026-09-21 — 🎯 **The camera rig feels like one: it avoids walls, holds a dead zone, leads the target, and every time it names is a duration it keeps.** #1251, #1252 and #1253 closed, with mouse look inside the input system (roll-a-ball#22), Ctrl+P play/stop with the panels and cursor following, gravity priorities and a PointGravity reduced to radius + falloff, and a toggled sprint. 🔴 **A field in seconds that does not last those seconds is a lie**: damping and soft zones were exponential time constants (63% at the stated time); they are now `Chase`, a tween that restarts while its goal moves and arrives exactly on time. 🔴 **A widget that reports a change on draw dirties the scene**: a ranged f32 slider snapped in f64 and wrote on every selection. **Next is #1258**, orbit from input and recentering. — Previously 2026-09-20 — 🎯 **A frame is a list of cameras, and a region decides the look.** #1220, #1221 and #1222 closed: a light lights and shadows the layers it names (and both shadow caches had to be invalidated when a mask changes in place), a base camera composes overlays that keep what they did not draw, and a post-process volume fades in over a sensor's depth. 🔴 **SGSR 2 wrote alpha 1 over the whole frame** — alpha is coverage, so an upscaled overlay composed as an opaque black plate with the base gone. 🔴 **A feature gated on a runtime resource is dead in the editor**: components cross the wire, resources do not, and the editor runs no solver — the volume worked in a built game and nowhere an author could look. A virtual camera now drives the camera that declares it with a `CameraBrain`, because "highest priority" hands the rig to an overlay. **Next is #1250**, the camera rig: occlusion, dead zone, lookahead — plus roll-a-ball#22, which has no camera input bound at all. — Previously 2026-09-20 — 🎯 **Transparency's last pieces, and a layer a camera can refuse.** #452 finished its masked raster (#1234) and turned a still coverage into a hull of a dozen triangles (#1236, #1239) — the exact contour cost four times the geometry and saved the same fill. `transparent_unlit` (#1241) is the blended kind no light touches. Noise tiles (#1238), so a polar-warped pattern has no seam. Named layers (#1242) are one table read by renderers and colliders alike, and a camera now draws only the layers it names (#1243) — rejected in the cull, with shadows deliberately exempt. **Next is #1220**, lights and shadows by layer. — Previously 2026-09-19 — 🎯 **The post-process reaches the built game, the graph edits like an editor, and a panel can live on another monitor.** #1201 closed (game window, graph preview, per-effect enable and weight), #1211 gave the Shader Graph undo, clipboard, groups and notes on an in-tree egui-snarl fork, #1196 tore panels off into OS windows that dock back where they were, and the Game view renders at any of the display's resolutions. 🔴 **The target pool leaked on every resize**: it never destroyed a texture, so each size a view passed through kept its target — hundreds while dragging a divider. It now frees what no pass claimed for three frames. #1197 was not a leak (an R64 view never allocates a Hi-Z pyramid). **Next is #452**, transparent. — Previously 2026-09-18 — 🎯 **A pass is a system, and a post-process is authored in the graph.** #392 closed in five PRs: systems order themselves by name (`Order::before` / `Order::after`), the dead `graph/` module is gone, a `GpuSystem` records into an encoder instead of a fixed compute pass, the game's frame is three ordered systems, render targets come from a `TargetPool`, and a plugin draws through `kooch_plugin_render` without `kooch_plugin_api` gaining a dependency. #1179 added the `unlit` kind; #1201 the `post_process` kind — a Scene Color node, a `PostProcess` component, applied in the View and Game panels under the gizmos — and #1204 made it a stack, which needed lists in reflection (`ReflectValue::List`) and `#[reflect(alias)]` so a renamed field keeps loading. 🔴 **The pool leaked until the machine went down**: it held released targets three frames and only the game's frame ever advanced them, so the editor allocated a viewport-sized texture every frame. The wait was never needed — the pool never destroys a texture, and radv's rule is for textures destroyed in flight — so reuse is immediate and a pool past 32 textures logs an error. 🔴 **And a feature the author cannot see in the editor is not done**: a Rust example plugin and tests only the agent can run proved the mechanism and nothing the author could judge. **Next, in order:** smoke two stacked post-processes (the author's, and a PS1 look built together in the graph), then #1201's game-window path and #1197, then #1196 (panels as real OS windows, dockable back), then #452. — Previously 2026-09-17 — 🎯 **#1159 closed: the shader graph is a finished tool, and its cost is measured on the handheld.** Typed parameter nodes (#1170), outputs whole and by channel, a full noise kit (fractal fBm/turbulence/ridged with roughness, lacunarity, distortion and 3D phase; Voronoi with metric, smoothness, true border and position), a UV category (polar, twirl, radial shear, spherize), and pins that name their width, carry Unity's port colours and explain themselves on hover. Each material's shading is its own GPU scope, so the graph header and the Profiler's **Shaders (GPU)** table show what a shader costs — locally and from a game running on the OneXFly. 🔴 **Voronoi smoothness tore**: a running polynomial smooth-min depends on cell order, measured on a CPU port at slopes of 800 and 1406 where a distance cannot exceed 1; a symmetric log-sum-exp over 5×5, capped at k 0.12, holds every output at ≤1.01. 🔴 **Blocks were missing from the built game** (#1187): `kooch/blockmesh` lived only in the `editor` feature, the scene loaded with its blocks parked, and the only trace was a WARN in a handheld's log — the editor now adds the engine features a project's scenes use, on open and before a build. The version bot was also leaving the root `kooch` package stale in `Cargo.lock` on every bump. **Next is #1189** — the Inspector wrote a material over `test_graph.shader`, pairing the new selection with the previous frame's detail — then the shader-kinds epic #1178 from #1179 (unlit). — 🎯 **The shader graph became a tool: 47 nodes, a preview on any primitive, and a graph that stays where you left it (#1159, #1166–#1169).** The library covers Input / Math / Vector / Effects / Shapes, so a material never needs WGSL; every node is written so an unconnected input cannot make a NaN, and `every_node_compiles` holds naga to each one with nothing wired. `SurfaceInput.time` rides in the padding the per-material screen UBO already carried — a shader got the clock without a binding. 🔴 **A preview does not need the renderer behind it, because it is the FRAME that lights a surface and never the surface itself**: `compose_preview_shader` rasterises one primitive with none of Inti, the contact march, the shadow pages or the visibility buffer, which is why it costs a thumbnail instead of a second viewport. 🔴 **And naga validates WGSL, not bind group layouts** — the preview's group 2 was built from the *deferred* path's layout (one binding, COMPUTE-only), so every pipeline was invalid on a real device while the suite stayed green; nothing but a GPU could have caught it. `egui-snarl` keeps its view in GLOBAL screen space and re-anchors it never, so the graph stayed behind whenever its panel moved (`current_transform` is the hook for it), and the panel's dirty check compared node *values* only, so dragging a node lost the layout in silence. Filed: **#1170** — typed parameter nodes (float, int, vecN, colour, texture), sliders from `@range`, colour pickers, textures assignable for the preview, and a resizable preview column. **Next is #1170**, once PR #1169 is merged. 🎯 **A material picks its shader, and a shader makes materials: Shader Forge phase 1 (#1157).** A `.shader` holds one WGSL function, `surface(SurfaceInput) -> SurfaceOutput`, that the fragment and compute frames both compose around; `Material.shader: None` keeps the engine's PBR surface, so every existing material renders unchanged. One pipeline per shader on both paths, validated by naga before any pipeline exists, and a broken save keeps the last good one. The compute path now bins by tile: each material dispatches indirect over the tiles it covers. 🔴 **The smoke test found moving tile-sized holes, and RADV was the reason: `atomicAdd` on a storage buffer is not atomic there.** Counters shared by many tiles lost updates, so lists came out short; every write now has one writer. The same smoke test fixed the asset and entity pickers (the search closed the popup, scrolled away and shrank it). #784 gains the shader kinds and the note that wgpu 29 has no geometry stage — grass and fur go through compute. **Next is #1158**, parameters a shader declares; triplanar (#1086) becomes a shader rather than a feature. 🎯 **Blocks start as parametric shapes and are shaped by dragging.** #1106: World → Spawn → Block offers cube, stairs (straight, turning, spiral), ramp, arch, cylinder, cone, plane and door; the block carries a reflected `BlockShape`, and changing a field regenerates it live. Editing the geometry by hand removes the component, so a parameter change cannot overwrite the edit. #1150: a handle per length on the selected block, one undo per drag on both paths. 🔴 **The smoke test found that the pivot pins its own face**: a handle placed there never moves, which is why the door's and arch's depth handles did nothing — every handle now sits opposite the pivot, and a test drags each one under pivots on every side. Filed from the same session: #1149 (UVs tear when a meshlet drops LOD), #1152 (shapes along splines, after Houdini and Blender), #1153 (combine blocks, confirmed first), #1154 (non-destructive edits over imported glTF). **Next is #1086**, triplanar, before #1069's glTF export. 🎯 **#1118 fixed: a click in an element mode selects another entity, and the smoke test found the second cause.** The `return` fell through only for a miss on empty space; that was not enough, because entity picking compared **bounding boxes** — a hollow or L-shaped block's box contains whatever stands inside it, so clicking the character selected the block around it, in Object mode too. Blocks now answer with the ray's distance to their **triangles**, and a 12 px vertex or edge pick no longer beats an entity nearer along the cursor ray. #1123 also closed: every crate's comments down to at most three lines, code lines identical in each PR. **Next is #1106**, primitive shapes. 🎯 **A block's vertices, edges and faces are selectable, and the grid the handles snap to is drawn.** The join that made it small: a face is its corners, an edge is two, a vertex is one — every edit below the selection already worked on a corner list, so vertex and edge needed no new transform, undo or save code. Picking did NOT come free: a vertex has no area and an edge no width, so "close enough" is a count of PIXELS, which only exist after the projection — godot-ply's `sqrt(distance)/32` is a curve fitted to the projection rather than the projection. 🔴 **The grid found an open circuit that had nothing to do with the grid**: it drew with `Greater` so geometry would occlude it, and still drew over everything, because the meshlet composite copied only COLOUR and the depth buffer every later pass tested against was the one the sky cleared. Sixth of that shape — a complete piece with an open circuit, silent because the absence had a legitimate reading. ⚠️ **Two of my own new tests passed under sabotage**; each guard now has exactly one test that goes red when removed. 🔴 **And a rule I wrote, cited as if it were the user's, was wrong**: "cero CPU readback" discarded GPU physics before measuring — a few hundred transforms are KILOBYTES, the cost is the sync point, not the transfer. The real rule is SEEK THE OPTIMISATION, NOT BLOCK THE FEATURE; discard with a number. Physics survey (#1116): Rapier never gets GPU (its maintainer said so), and on AMD in Rust **Nexus is the only option** — so waiting costs nothing. **Next is #1118**, found in the smoke test: in an element mode a click can never select another entity, because a `return` I defended when Face was the only mode now fires on the case it was written to exclude. 🎯 **The level tool builds a level: faces picked, moved and EXTRUDED, undoable, colliding, walked on.** #946's first slice is closed. Extrude's real case is several contiguous faces as ONE piece, and it falls out of one count — an edge used twice inside the selection is interior, used once it is the rim. The direction is the AVERAGED normal, because per-face tears a curved patch into a fan with nowhere for its walls to meet. 🔴 **Five bugs this cycle shared one shape: a complete piece with an open circuit, silent because its absence had a legitimate reading** — `GeneratedMeshes` inserted nowhere read as "nothing to upload", `ReloadedAssets` with three readers and no writer read as "nothing changed", `SHAPE_OWN_MESH` listed as mesh-derived but missing from `from_mesh` fetched its geometry and dropped it on the last line. **Check a seam is REACHED before checking it is correct.** Its twin: **there are always two paths**, local and over the wire, and three features shipped fixed on one side only. **Next is #1070** — the handles snap to a grid nothing draws, and it carries an unanswered question about which plane it lives on. 🎯 **The block tool draws and collides; #946's first slice is half done.** Five of seven PRs fixed something the previous one broke, and every failure had one shape: **an absent thing reading exactly like an empty one**. A format named after its syntax (`.blockmesh.ron`) collided with `Material`'s `ron` and the scan's `find` took the first loader, so a block was typed as a material — now `.block` and `.material`, with `no_two_loaders_claim_one_extension` guarding it. `GeneratedMeshes`, the one store a generated mesh reaches the GPU through, **was never inserted anywhere**: every block built its mesh and dropped it, and `if let Some(...)` on both sides meant nothing failed. A GUID naming a `.block` was handed to a glTF parser by both generic walks, and `ColliderMeshCache::fail` is **permanent**, so the body never collided even after the real entry arrived. **Next is #1079** — adjacency, derived and never stored — then #1080 selection and #1081 extrude, in that order because each needs the one before. 🎯 **Next is #946, and it was rescoped from CSG to a ProBuilder clone — because ProBuilder is the SMALLER problem.** The whole difficulty of CSG is mesh booleans, and the dependency research is a tour of that graveyard: `manifold-csg` is the state of the art and is C++ behind bindings, which would put a new requirement on every machine that builds a project; `csgrs`'s `main` depends on eight unpublished sibling crates **by path** and has moved off Dimforge entirely, leaving only a thirteen-month-old release that wants `nalgebra 0.33` against this engine's 0.35. **Extrude, inset, bevel and bridge are LOCAL topology edits — no boolean, no robustness cliff, no dependency to pick.** The feature list is an order of magnitude bigger; the core is three things, and the first is `BlockMesh`: an authoring mesh beside the render one, because `Mesh` duplicates positions per face and cannot be edited in place. 🔴 **ProBuilder is under the Unity Companion License, which is not open source** — read it, never copy it into this repo. — 🔴 **Hot reload of Rust is closed, with the record, so it is not relitigated.** Nobody has solved changing a struct without compiling it, and the three implementations that exist all say so in their own documentation: `subsecond` *"does not support hot-reloading of structs"*, `hot-lib-reloader` calls a layout change *"undefined behavior which will likely result in a crash"*, and **Fyrox** — the one Rust engine shipping code hot reloading — calls its own *"wildly unsafe"*, warns it can miss plugin entities and *"result in crash or memory corruption"*, and **says nothing at all about adding a field to a component**. `dexterous_developer` was archived in February. **crABI is not the answer either, and that is the finding worth keeping**: a stable ABI fixes the layout of a *given declaration* across compilers, not the compatibility of a *changed* one — C has had a stable ABI for forty years and still cannot add a field to a struct in a shared library. It is also stalled: RFC 3470 open since August 2023, the implementation PR since December 2022, human comments on it ending in **December 2022**. **The engine stays full Rust**; #733, #121 and #75 are closed with their reasoning in their bodies. — 🎉 **the rebuild loop, in six PRs, and the finding was that the engine already held most of it.** `subsecond` is refuted for this project on three independent grounds — it cannot change a struct, it only patches the crate holding `main.rs`, and it needs Dioxus' CLI and linker — and `dexterous_developer` was archived in February. What every serious implementation converges on instead, this engine had already built for another reason: `DynamicTypeRegistry` keyed by NAME, `DynamicComponents` holding instances as `ReflectValue`, and an `unload_project_plugins` **with zero callers**. Measured before trusting: a 792 MB project library unmaps completely and reopens clean, and the swap is **38 ms**. 🔴 **Then play testing refuted half of it**: in remote mode the wire's schema correctly shadows the reloaded local registry, so the in-process swap is invisible to the author — seeing a new field genuinely requires the project's process to run new code, and there is no way around the compile. What was actually costing the loop was the two things beside it: a rebuild **lost the world** (fixed, #1055 — held on disk and put back, still dirty) and it takes **15.5 s**, nearly all of it linking a 645 MB binary with the default linker. — ⚠️ **#815 landed, and the interesting half is that half of it was already done.** The asset browser has refused Delete on the engine's tree since it drew one: `writable` is `false` for that root and both context menus gate on it. The *handler* asked nothing — `delete_asset` called `remove_file` and `delete_folder` called `remove_dir_all` on whatever path arrived. **Enforcement lived in the drawing code, which is the layer that can only refuse the clicks it happens to draw.** Twenty lines above them `workspace_for` already told a project path from an engine one, under a comment reading *"Engine assets are read-only"* — a promise no code kept. The guard now sits in front of both removals and asks the project first, because an editor built from a project resolves its engine root to that same project and engine-first would turn every asset in it undeletable. — 🎉 **#94 landed: the character controller, and it took eight rounds of play testing to become one.** Every fix in it was found by *playing* and then measured by reverting: a jump that rose 0.145 m of a 1.27 m arc, a landing that dipped 0.000 m, a character still doing 1 m/s a second after the stick was released, a body leaning 28.6° into a wall it was shoving, a climb that lost the ground on 172 frames out of 180, a 65° slope walked up from y=1.0 to y=20.0, and a wall run that never saw the wall it was running along on 60 frames out of 60. **None of that was visible in a test until someone played it, and none of it was ambiguous once it was.** The through-line is one idea: **one pass senses, many mechanics read** — `sense::under` and `sense::beside` cast, `Grounded` and `Touching` carry the answer, and `Walk`/`Sprint`/`Jump`/`WallJump`/`WallSlide`/`WallRun` decide. Two systems each casting their own ground ray is how `apply_movement` and `apply_jump` came to be wrong in the *same* way, both reading "grounded" in mid-air, because rapier casts rays solid and a downward one from a body's own centre finds that body at zero distance. **Next is #815** — an engine asset can be deleted and it breaks every project on the machine, not just the open one. — 🔴 **the standing queue was mostly already done**: of the five issues in the 2026-08-30 order, three closed by reading the tree, and two of those had been fixed with nobody recording it. Verify against the code before scheduling. **Next is #719** — a scene naming an unknown component type loads silently, which already cost a shipped build every `Spin` pivot. — 🔴 **#1009 landed: the lamps cast again.** Four fixed-size caps deleted them in silence — a dispatch past 65 535 workgroups, a 16 384 pair list, a per-view clear over per-frame counts, and a 256-caster moved list that voided the page cache every frame. Each failed as a resident, correctly keyed, EMPTY page, which reads as lit. The distant tier is derived from `page_level` rather than tuned. **Next is the OneXFly measurement** — 5.36 ms on a 9070 XT decides nothing. — 🔴 **the bright patch is fixed: it was Olsson's receiver bound, and the debug view had no colour for it.** A page drawn *without one caster* paints green, the same as a bad bias, so four correct eliminations pointed nowhere before the bound was tried. #1022 landed and is worth its claim — `geometry walk 245` against `pair tests 452 432`. **Next is #1009**, the distant-light tier: 32 point lights exhaust the pool (`slice used 1024/1024`, `free 0`) because a lamp is either a full chain or nothing, and Unreal's middle tier — one page per distant light, round-robin at one update a frame — is what is missing. — 🔴 **there is no render distance in this engine.** `perspective_infinite_reverse_rh` never receives `PerspectiveCamera::far`, and no `draw_distance`/`cull_distance` exists anywhere in the tree, so every instance in a scene is in frustum forever — free at 190 m, the whole frame at 1410 m. Two silent limits cost a day each (#996 buffer sizes, #997 the 65 535 dispatch ceiling) and `dense.scene` opened; what it showed is in the order below. 🔴 **a frame cap is a PERFORMANCE setting on this part: the same work costs 3.9 ms of GPU capped at 72 fps and 13.2 ms uncapped, because capped the GPU idles 68 % of the time and holds ~1210 MHz instead of throttling to ~850.** `gpu_busy_percent` reads 32 % and the scopes agree. **The budget is met with the preset below** — 13.88 ms frame, and at 8 W capped only **28 % of it is used**. The lever was the upscaler, not the shading: `upscale: 3` (FSR 3.1) cost 11.355 ms of a 23.36 ms frame and `upscale: 2` (SGSR 2) costs 2.062. 🔴 **The ~11 ms shading floor #885 was built to decompose no longer exists** — the whole shading pass is 3.272 ms today — and the instrument built for it measured something else instead: a full-screen sweep per material **in the project** costs 178 µs on the device, which is 0.71 ms here and 3.7 ms for a game with twenty materials. **#826 is removed, not deferred.** Cutting a froxel's light list by COUNT is incompatible with a cluster grid being continuous, and that is a property of the idea rather than of any implementation of it: see the entry below. The remaining queue is the contact march's cap (#839) and #731. The budget is unchanged, still unmet, and now measured against the SETTLED clock rather than the boosted one — 40.7 ms, not 27.8.
...
## 🎯 2026-09-21 — the camera rig feels like one

### What shipped
- **Input:** mouse motion is a control path read as a velocity (#1267), with cursor capture; roll-a-ball turns its camera from mouse and right stick (roll-a-ball#22/#23) and sprints (#24).
- **Editor:** Ctrl+P plays and stops, the Game/Edit views come forward and the cursor follows (#1270); a prefab edits project components (#1268); Ctrl+S saves the active scene to its own file and Save As opens beside it, every outcome in the Console (#1277).
- **Render:** the sky no longer kills transparency — colour and coverage stay premultiplied through the blit (#1269).
- **Gravity:** every source has a priority that suppresses lower ones inside its reach (#1271); `PointGravity` is `radius` + `falloff`, migrated by alias (#1272).
- **Character:** `Sprint.toggle` — a press starts it, stopping ends it (#1278).
- **Camera rig (#1250):** `CameraCollision` sweeps a sphere and returns on a tween (#1275, #1276, which also made `kooch_ecs::tween` global); `CameraFraming` dead/soft zones drawn over the Game view (#1281); `CameraLookahead` (#1285); every time field is `*_duration` and exact (#1283).
- **Versioning:** pre-1.0 shift in the bot (breaking → minor, feat/fix → patch) and a body marker, because a label races the `opened` event (#1273, #1274).

### 🔴 What it taught
- **A setting named in seconds must last those seconds.** An exponential "time constant" closed 63% in the stated time; renaming it did not fix it. `Chase` restarts from the current value whenever the goal moves, so it arrives exactly `duration` after the goal stops, at any frame rate. Ease-out is fixed: an ease-in never leaves a moving goal.
- **A widget that reports a change on draw is a write on select.** `0.2f32` snapped to a 0.01 step in f64 is not `0.2f32`; selecting a vcam dirtied its scene. `idle_tests` draws components with no input and fails on any edit.
- **A silent cancel reads as a broken save.** The dialog was rooted at a folder that does not exist, so the portal opened elsewhere and closing it said nothing.
- **Check the branch head after the version bot runs, before merging** — `feat!` produced 1.0.0 once.

### Filed
**#1279** `kooch_locomotion` (Grounded, Touching, Jump, Sprint for any body — vehicles), **#1280** a character that rides, grips and bounces off what it stands on, **#1284** scaled, unscaled and per-entity time.

### Next
1. **#1258** — orbit from input and recentering (a target at rest drifts back to `screen` after a while).
2. **#1260** zones that switch the rig, then **#1261 → #1262** splines and rails.
3. Candidate, not filed: edit camera components while playing (today every world edit is refused in Play, and zones are tuned by looking).
4. Still open and unscheduled: **#1247**, **#1249**, **#1240**.

---

## 🎯 2026-09-20 (later) — a frame is a list of cameras, and a region decides the look

### What shipped
- **#1245 — #1220 closed.** A light lights the layers it names, tested per pixel at the top of `inti_light_lit`, and casts only what `shadow_layers` names, rejected in that light's own cull — cascades, spot maps, cube faces and the virtual pages alike. 🔴 Both shadow machines **cache**: a mask edited in place kept the old shadow on screen until `CubeKey` carried it and the page machinery was released when any mask changes.
- **#1246 — #1221 closed.** Camera stacking: a base camera owns the image, every `overlay` draws over it in priority order, each a view of its own with its own culling mask and temporal history. The composite is the blit that already existed, because the stage leaves alpha at 0 where nothing was drawn. 🔴 **SGSR 2 wrote alpha 1 over the whole image**, so an upscaled overlay composed as an opaque black plate with the base gone — alpha is coverage, and every pass that rewrites colour has to carry it.
- **`CameraBrain`** — a virtual camera drives the camera that *declares* it. "The highest-priority camera" was the rule while a frame was one camera; with a stack it handed the rig to whatever overlay outranked the base.
- **#1248 — #1222 closed.** Post-process volumes: the shape is a sensor collider, the solver is the gate, `SensorOccupancy` is the stay between its arrival and its departure, and the depth is the blend — smoothstepped, in metres, folded over the scene's own stack in priority order by Unity's rule, so a volume can turn an effect **down**.
- A region **authors itself** (a fixed sensor that reports, from the component alone), **is every collider under it** (the solver's compound, deepest answers), and is measured **without a solver** where one does not exist — the editor runs none.

### 🔴 What it taught
- **A feature gated on a runtime resource is dead in the editor.** The editor adds the components-only physics plugin: no solver, no events, nothing to fill an occupancy. Components cross the wire; resources do not. A volume worked in a built game and nowhere an author could see it.
- **Three silent switches in a row.** A sensor needs a body to exist, `sensor` to overlap, `collision_events` to be heard — and each one fails without a word. Rapier's default pairs also leave out `KINEMATIC_FIXED`, so a fixed trigger never heard a kinematic character.
- **Measure in the units the author types.** The depth was in shape units and the blend distance in metres: every scaled region blended by its scale factor too fast, and only a box at scale 1 agreed.
- **A test that passes under sabotage is not a test.** The upscaled-overlay case passed while the bug was in, because it only asserted the base survived — an overlay that composed *nothing* satisfies that.
- **`draw_ranged` returned the change rather than whether it drew**, so every ranged field in the Inspector drew a slider *and* a drag value, every frame nobody touched it.

### Next
1. **#1250** — the camera rig epic, and the session it was scoped for: **#1251** occlusion, **#1252** dead zone and soft zone, **#1253** lookahead, in that order. Then **#1254** the lens on the vcam, **#1258** orbit from input and recentering.
2. **roll-a-ball#22** — the camera has no input bound to it at all: mouse and right stick, in the test project, which is where the rig is judged.
3. Open and unscheduled: **#1247** render texture cameras, **#1249** an overlay that keeps the base's depth, **#1240** transparency on the fragment path.

---

## 🎯 2026-09-20 — transparency finishes, and a layer is something a camera can refuse

### What shipped
- **#1234 — #452 step 2** — masked materials raster in a bin of their own, as Nanite bins a programmable raster: one pipeline per masked shader composes `surface()` and discards below `alpha_clip`, on both visibility buffers, with the shadow bake read as a hard cut. A transparent shader can clip too: what falls below the clip takes none of the pixel's four layers, and the rest blends as it always did.
- **#1236, #1239 — #452 step 2b** — a coverage that cannot move becomes geometry. The alpha is baked once over its uv square, and the mesh is cut down to a **hull** around it: deliberately coarse, always wider than the coverage, with the material still cutting per pixel inside. Tracing the alpha exactly cost 63 triangles for a rounded rectangle; the hull costs 15 and drops 58% of the fill. A budget refuses the cut when it would cost more than it saves.
- **#1241 — `transparent_unlit`** — the blended kind no light touches, for fire, energy and particles. Every router now asks `ShaderKind::blends()`.
- **#1238 — #1237 closed.** Noise tiles: a `tiling` input wraps the lattice before it is hashed, so a uv that comes back to itself — a tiled texture, a full turn of polar coordinates — has no seam. Simplex refuses it rather than lying about its skewed lattice. The seam is measured off the GPU by baking the surface over its own uv and comparing the step across the edge with the steps inside.
- **#1242 — #1218 closed.** Named render layers: 32 names in a project's `.layers` file, edited as a table, read by a renderer's mask and by a collider's four group masks alike. `#[reflect(layers)]` marks a field, and the Inspector draws a dropdown that says what the mask holds.
- **#1243 — #1219 closed.** A camera draws the layers it names, rejected in `skipped()` — the one place every cull path already asked whether an instance is this view's. Shadows are not filtered by it: they belong to the light.
- **Debug views cover the window**, and a **Wireframe** view — with a variant drawn over the shaded frame — makes a polygon budget something you can look at.

### 🔴 What it taught
- **An optimisation that traces the alpha is not an optimisation.** The first cut followed the contour exactly and turned a two-triangle card into sixty-three. What saves fill is a hull of a dozen corners that *contains* the coverage; the material keeps cutting inside it. Dilate by a margin, simplify by no more than that margin, and the containment is a property rather than a hope.
- **A number in the log is the difference between "it works" and "is that good?"** The trim's line now carries the share of the fill that is left.
- **An immediate-mode field rebuilt from its model every frame cannot be typed into.** The layer names could not be changed at all until the in-progress text lived in the widget's own memory.
- **Asking a response for its focus inside `data_mut` deadlocks** — the same lock, ten seconds, then egui's own detector.
- **The editor is where a graphics feature is judged**: three of this session's bugs were found by looking, and the tests were written afterwards to keep them found.

### Next
1. **#1220** — a light lights and shadows only its layers, tested per pixel in the shade and per caster in the page raster. The issue asks for the cost to be measured on the OneXFly.
2. **#1221** camera stacking, **#1222** post-process volumes — the rest of the layers block.
3. Then: the camera system's own audit, which is what the layers block was a way into.
4. Open: **#1240** — transparency on the fragment shading path, which asks first whether that path is ever shipped.

---

## 🎯 2026-09-19 — the post-process ships, the graph edits, panels leave the window

### What shipped
- **#1206–#1208** — post-process shaders kept out of the surface pipeline cache, a quiet console, a project's dependencies optimised in its dev build (the editor runs the project in dev), and scalar graph outputs splatted (`colour × Float` was red).
- **#1210 (#1209)** — `PostEffect { material, enabled, weight }`: each effect switches and blends on its own. Reflection gained lists of structs (`ReflectValue::Struct`, `FieldMeta::fields`, `#[reflect(bare)]`).
- **#1212 — #1201 closed.** The game window draws into a pooled target when a stack is active and copies the result onto the swapchain; the graph panel previews a post-process over a generated test image.
- **#1213, #1214 — #1211 closed.** Shader Graph undo/redo per document, copy/paste/duplicate, shortcuts with an on-screen sheet, groups with explicit members that fit their nodes, renamable in place, and notes. egui-snarl is an in-tree fork (`crates/egui_snarl`, licences and `KOOCH.md` kept), updated with `update_egui_snarl.py`.
- **#1215 — #1196 closed.** *Open in window* on any tab: the panel becomes an egui immediate viewport of the main context in a native window, keeps its state, draws at that monitor's scale, and docks back to the leaf or split it came from at its old size. Torn-off panels are saved with the layout. `kooch_window::ExtraWindows` lets a system ask the event loop for a window.
- **#1215** — `TargetPool::end_frame` frees targets unclaimed for three frames.
- **#1216** — the Game view renders at Free (its panel) or any display resolution, letterboxed to fit. `DisplayModes` is published in the editor too; a game changes its own resolution through the `Resolution` resource.
- **#1197 closed, not planned** — an R64 view never allocates Hi-Z pyramids, so nothing is retired on resize.

### 🔴 What it taught
- **egui backends own three defaults eframe sets for you**: `set_embed_viewports(false)` (otherwise a viewport is a window *inside* the main one), a nested pass must carry the main pass's `max_texture_side` (a different one rebuilds the font atlas under the main window and garbles every font), and a secondary surface must not present FIFO (two vsync waits halved the frame rate).
- **A pool that never destroys is a pool that leaks on resize.** Reuse by descriptor settles a steady view at one target and a dragged one at hundreds.
- **egui menus close on a click inside by default** — a text field in a context menu needs `CloseOnClickOutside`.
- **Two visible views cost two renders.** Game and Edit side by side is 200 → 80 fps at full-screen size; a fixed lower Game resolution is the lever.

### Next
1. **#452** — transparent / alpha clip, starting with the forward pass as an ordered system.
2. Open: several panels in one OS window (a dock per window, with *Move to window ›*) — waiting on a decision.
3. #1195 — the graph preview's size and rotation.

---

## 🎯 2026-09-18 — a pass is a system, and the graph writes post-processes

### What shipped
- **#392 closed.** `Order::before/after` by system name, sorted at registration; `graph/` deleted; `GpuSystem::record(frame, encoder)`; the game frame as `prepare_frame_system` → `render_meshlets_system` → `present_frame_system`; `TargetPool` by descriptor; `kooch_plugin_render` + `engine.add_pass`.
- **#1189** — the Inspector could write a material over another asset; the detail now carries its guid.
- **#1179** — `unlit` kind, picked on the Output node.
- **#1201 (part)** — `post_process` kind, Scene Color node, `PostProcess` component, applied in the editor's viewports.
- **#1204** — post-process stacking; `ReflectValue::List`, `#[reflect(alias)]`, a list widget with reorder.

### 🔴 What it taught
- **A pool that waits to reuse must be told when a frame ends, and one caller forgot.** The wait bought nothing — the pool never destroys a texture — so it went, and the pool now shouts past 32 textures.
- **Tests the author cannot run prove nothing to the author.** Every graphics feature ships with a way to see it in the editor.

### Next
1. Smoke two stacked post-processes: the author's, and a **PS1 look** built in the graph — low resolution (pixelate by snapping the Scene Color uv), 15-bit colour (5 bits per channel), 4×4 Bayer dithering. Vertex jitter and affine texture warping are *surface* effects, not post-process ones (#1180 vertex offset).
2. **#1201** — the game window (the swapchain image is not sampleable, so the same pooled detour) and a preview of the kind in the node panel. Until then a post-process does **not** show in a built game.
3. **#1197** — Hi-Z pyramids leak on the R64 path at every resize.
4. **#1196** — panels as real OS windows, movable to another monitor and dockable back. Not small: this editor drives `egui-winit` + `wgpu` itself, so each window is its own surface, egui state and present.
5. **#452** — transparent / alpha clip, starting with the forward pass as an ordered system.

## 🎯 2026-09-17 — #1159 closed, and what a shader costs is measured on the handheld

### What shipped
- **Nodes**:
  - typed parameters and constants (#1170);
  - outputs whole and per channel;
  - the noise kit (#1175);
  - a UV category (#1176);
  - pins with width suffixes, Unity port colours, tooltips and category-tinted headers (#1177).
- **Measured shader cost** (#1185):
  - `GpuScopes::begin_in` times inside a pass, and `totals()` sums by label;
  - both shading paths open a `shader <guid>` scope per material;
  - the graph header shows `N nodes · X ms GPU`, and the Profiler's Shaders (GPU) table reads the local frame or the game's over the network. Tested on the OneXFly.
- **Scene features in builds** (#1187, PR #1188): `build/scene_features.rs` reads every scene's `kooch_*` component types against the project and engine `[features]`, and adds the missing `kooch/<feature>` to `game` on project open and at build start. `bump_version.py` now retags the root package and stale lock entries.

### 🔴 What it taught
- **A smooth minimum must be symmetric.** IQ's running polynomial blend depends on the order cells are visited, so F2 and the border tore where cells swapped. Measured on a numpy port before touching WGSL: log-sum-exp over 5×5 holds slope ≤1.01 up to k=0.12, and past that the 5×5 window itself tears.
- **An unknown component in a shipped scene is a missing level, not a warning.** The load is correctly non-fatal; the failure is that nothing upstream stopped the build.
- **A frame's selection and a frame's detail are two different frames** (#1189). The Inspector sent a material edit with the next selection's guid and overwrote a shader. A handler that writes files must check the asset type it was given.
- **A test that passes under sabotage matched the wrong line**: the on-disk features test found `"kooch/blockmesh"` at the end of the `editor` line. Now it checks the `game` line.

### Decided
- **Shader kinds get an epic, #1178, with one issue per kind**:
  - unlit #1179;
  - transparent #452;
  - compute scatter #1181;
  - sprite #1183;
  - particles #1184, with Start and Process graphs as in Godot, compiled to #1115's simulation;
  - vertex offset #1180;
  - and a grass shader #1182 as the proof.
- **There is no geometry kind**: WGSL has no such stage, so compute plus indirect draw does its job.
- **Everything must work on a dual-contouring planet**:
  - the local up is `gravity_up`;
  - no heightmaps, no terrain UVs (use triplanar);
  - coordinates relative to the chunk or the floating origin;
  - particles obey `kooch_gravity`'s sources, evaluated in WGSL and pinned equal to `gravity_at` by a test.
- **Wind (#978) is four layers**:
  1. a planetary cubed-sphere;
  2. analytic curl noise;
  3. painted local flowmaps and bounded 3D flow fields, imported from `.fga` or `.vdb` with Blender as the free reference source, plus calm zones;
  4. a camera cascade that is visual only and never feeds physics.

### Next
#1189, then #1179 unlit.

---

## 🎯 2026-09-16 — the shader graph became a tool

### What shipped

- **#1166 — the node library.** 47 nodes, grouped in the menu: Input (including **Time**), Math, Vector, Effects (fresnel, normal-map unpack, panner, rotator, tiling, desaturate, blend modes, noise) and Shapes (circle, rectangle, ring, polygon, checker). WGSL helpers are written into the file **only when a node asks for one**, so a generated shader carries nothing it does not use.
- **#1167 — navigating it.** The view follows its panel, a minimap (Godot's `GraphEditMinimap` shape: fit the bounds, never stretch), and auto-arrange after Godot's `GraphEditArranger` — layer by longest path, order each layer by the median of what feeds it. Its inner-shift and block-placement passes are left out: they earn their keep on graphs far larger than a shader's.
- **#1169 (open) — the preview.** The graph's shader on any of `Primitive::CANONICAL`, turning, in a column beside the nodes. Tangents are computed per primitive because `Mesh` carries none and **Unpack Normal** previews as noise without one.

### 🔴 What it taught

- **It is the frame that lights a surface, never the surface itself.** That one line of the contract is why a preview needs neither Inti nor the visibility buffer, and it turned "half a meshlet pipeline per panel" into a WGSL file and a pass.
- **naga validates WGSL, not bind group layouts.** A layout is only checked when a real device builds the pipeline, so `the_preview_frame_compiles` passed while the GPU rejected every frame. There is no test for this class short of a device.
- **A dirty check that compares values only loses everything else.** The panel decided the graph was unsaved by comparing node values, so dragging a node — or arranging the whole graph — was thrown away on close without an asterisk.
- **`@color` is a `vec4` hint**, and the graph emitted it on a three-wide parameter: the file was written, failed to reload, and the Inspector went on showing the parameters from before it. `generate` now parses what it produced before returning it.

### Next

#1170 — typed parameter nodes with the editors they deserve — then #452 (transparent) for what genuinely needs a second pass. #1159 still owes the node count and shader cost in the profiler.

---

## 🎯 2026-09-15 — Shader Forge phase 1: a material picks its shader

User's decision (#784, option A): **a pipeline per shader on both shading paths**, never per material, and custom shaders are not demoted to the fragment path.

### What shipped (#1157, PR #1160)

- `.shader` asset: WGSL with a `// kind: surface` header. `material_surface.wgsl` is the contract (material bindings, `SurfaceInput`, `SurfaceOutput`); `material_frame_fragment.wgsl` and `material_frame_compute.wgsl` call `surface()`, so one file runs on both paths.
- `ShaderPipelines` caches by shader and debug variant and rebuilds when `ReloadedAssets` moves. `validate_surface` runs naga against both frames and reports the line in the author's file.
- Compute tile binning (`material_tile_bins.wgsl`) with indirect dispatch per material, no readback; `KOOCH_TILE_BINS=off` is the A/B.
- Editor: New Shader, right-click a `.shader` → Create Material, a Shader picker, and an mtime poll that hot-reloads shaders saved from an IDE. Pickers share `search_combo`.

### 🔴 RADV does not make buffer atomics atomic

`atomicAdd`/`atomicOr` on a storage buffer lost updates under contention, and a test binning one frame in isolation passed every time — the holes appeared only in the full stage, intermittently. Every write in the binning now has exactly one writer. The compute frame's older `atomicMin`/`atomicMax` over `var<workgroup>` (#824) is the same class and is not yet checked.

### Decided in #784

Shader kinds on one asset: `surface` (#1157), `transparent` (#452), `post_process` (#254, #484), `particles` (#1115), `sky`/`fog` (#976, #977), `geometry` for grass and fur — generated by compute and drawn indirect, because wgpu 29 has no geometry stage. Coverage and position changes need a depth entry the VSM page raster also runs.

### Next

#1158 shader-declared parameters, then #1159 the graph. Then #1069 glTF export, #1149 UV LOD seams, #1087, #1153, #1154, #1152.

## 🎯 2026-09-10 — the three modes stopped being three things, and the survey said wait

#1070 and #1080 both landed. A block's vertices, edges and faces are selectable and
draggable, and the grid the handles snap to is drawn, infinite, and sits **in** the
scene rather than over it.

### One join made vertex and edge nearly free

A face is its corners, an edge is two of them, a vertex is one — and every edit below
the selection already worked on a **corner list**. `corners_of(mesh, mode, elements)`
is the whole join: no new transform code, no new undo path, no new save.

Picking did not come free. A vertex has no area and an edge no width, so a ray misses
both every time; "close enough" for them is a count of **pixels**, and pixels only
exist after the projection. godot-ply (MIT) approximates it with a world radius growing
as `sqrt(distance) / 32` — a curve fitted to the projection rather than the projection,
and it drifts with field of view. `vertex_at` / `edge_at` take the clip matrix instead
and measure there. Nearest to the cursor wins, ties broken by depth: **both halves earn
their place**, because a cube's front and back corners project onto the same pixel, and
depth alone hands a click to whatever is nearest the eye even when you aimed twenty
pixels away from it.

### 🔴 The grid found an open circuit that had nothing to do with the grid

The first grid was a line list. That could never work — a grid of segments ends
somewhere and it can only shade **per line**, so one near the centre stays bright all
the way to its far end. Replaced with a fullscreen pass deriving the lattice from each
pixel's world position, adapted from Godot's editor grid: the level comes from
`log(distance) / log(steps)` and **the fractional part is a crossfade**, plus an angle
fade and a distance fade measured **from the camera**, not the origin.

It drew with `CompareFunction::Greater` so a block in front would occlude it. It still
drew over everything.

**The meshlet composite copied only colour.** `depth_stencil: None` on the blit, so the
depth buffer every later pass tested against was the one the sky cleared. The `Greater`
test was comparing against nothing.

Same shape as the five before it — **a complete piece with an open circuit, silent
because the absence had a legitimate reading** ("gizmos draw on top, that's the point").
It was true, for the lines. Nobody had ever needed that buffer to be true. Fixing it
fixes every later pass, and it is what makes particle-vs-depth collision reachable at
all.

### ⚠️ Two tests passed under sabotage

Written to cover the new picking, and neither measured what its name claimed: the
`w > 0` guard and the depth tie-break could both be deleted with the suite green.
Replaced, and each guard now has exactly one test that goes red when it is removed.
**Reverting the fix is the only thing that distinguishes a test from a decoration.**

Nine new tests also shifted the parallel scheduling enough to expose an old race:
`carry::capture` clears one machine-wide directory and `carry::resume` deletes it.

### The physics survey, and why it ends in "wait"

Asked whether debris could be GPU physics. **Rapier never will be** — its maintainer
closed `rapier#829` saying so on 2026-08-08, one month before the question. Dimforge's
answer is **Nexus**, Apache-2.0, GPU rigid bodies through wgpu.

The wider survey (#1116) ends narrower than it looked, because one filter kills the
field: **the targets are all AMD**. PhysX 5 is genuinely BSD-3-Clause now — the surprise
of the survey — and needs a Volta-class NVIDIA card, with a Rust binding stale since
2023. Warp is CUDA-only. Bullet's OpenCL pipeline last changed in **2020**. Jolt and
Avian are excellent and are CPU.

**On AMD, in Rust, Nexus is the only one.** Which removes the pressure to decide: there
is no better option waiting to be found, so waiting costs nothing. #1117 is the
investigation, framed as a spike **beside** `PhysicsBackend` rather than an
implementation of it — that trait's 34 accessors are synchronous, and a naive adapter
would measure the adapter.

### 🔴 A rule I wrote, cited as if it were the user's, and it was wrong

*"Cero CPU readback en hot path"* sat in the rules file and I used it to discard GPU
physics before measuring anything. The user's correction: he never wrote it, and the
real rule is to **seek the optimisation, not block the feature**.

He was right on the engineering too. A few hundred transforms are **kilobytes** — the
transfer was always free. The cost is the **sync point**: blocking the CPU on a fence.
Deferred one frame, or staged through a ring, the same readback is nearly free. **Size
and timing, not permitted and forbidden.**

The rule is removed. What is worth keeping is the shape: a rule that begins with
"zero", "never" or "forbidden" is applied without measuring, and that is where it stops
being engineering. If something has to be discarded, discard it **with a number**.

### Filed, so the queue is written down rather than remembered

#1106 primitive shapes (stairs — godot-ply has almost none of this) · #1107 snap to
nearby geometry · #1108 loop and ring select · #1109 box select · #1110 per-face
material · #1111 screen-space displacement · #1112 lattice cage · #1113 runtime
deformation · #1114 structural destruction · #1115 GPU particles · #1116/#1117 the
physics survey. #1095 folded into #784, which was the same feature filed twice.

### Next

✅ **#1118 — fixed (#1146).** ✅ **#1106 — parametric shapes (#1148)** and ✅ **#1150 — shape handles (#1151).**

Next: #1086 triplanar **before** #1069's glTF export, or the artist inherits stretched UVs on every off-axis face. Then #1149 (UVs tear across meshlet LODs — the simplifier weighs positions only), #1087's free list, #1153 combine, #1154 non-destructive glTF edits, and #1152 spline shapes planned with #1112/#1113.

---

## 🎯 2026-09-09 — the level tool builds a level, and five bugs shared one shape

#946's first slice is closed. A block is spawned, its faces picked, moved,
rotated, scaled and **extruded**, the shape is undoable, it collides, and the
character walks on it.

### What extrude actually needed

Extruding one face is easy. Several contiguous ones **as one piece** is the case
the feature exists for — a 2×3 patch of floor pulled up is a wall, not six
pillars — and it falls out of one count: an edge used **twice** inside the
selection is interior and gets no wall; used **once**, it is the rim. A selection
in two disconnected pieces extrudes as two, with no case for it.

🔴 **The direction is the averaged normal.** Per face tears a curved patch into a
fan of disconnected pieces with nowhere for the walls between them to meet. A
selection facing two ways answers `None` rather than sending half of it
backwards.

`godot-ply` does the same walk over a winged-edge, and about a hundred of its
lines are the bookkeeping — `edge_cw`, LEFT/RIGHT sides, remapping every edge
and vertex through two dictionaries. It needs them because its faces are
**implicit**. `BlockMesh` stores faces explicitly, so enumerating one is a slice
and all of that disappears. **The algorithm it teaches is the same; the
machinery is not.**

### 🔴 Five bugs, one shape

Every one was a complete piece with an **open circuit**, and every one was silent
because its absence had a legitimate reading:

| what | read as |
|---|---|
| `GeneratedMeshes` inserted nowhere | "nothing to upload" |
| `ReloadedAssets` — three readers, no writer | "nothing changed" |
| `prototype_material` called by nothing | "no material" |
| `PhysicsBody` missing from the remote spawn | a default component |
| `SHAPE_OWN_MESH` in `MESH_DERIVED` but not `from_mesh` | `_ => None` |

The last one is the sharpest: the walk **fetched** the geometry, resolved the
cache, carried it all the way in, and dropped it on the last line. Everything
upstream was working.

⚠️ **Check that a new seam is REACHED before checking that it is correct.**

### And its twin: there are always two paths

Local, and over the wire through `remote_edit::dispatch`. Three features shipped
fixed on one side only — `SpawnBlock`, `PhysicsBody`, `BlockEdit` — because an
absent component reads as a default and an unrouted action falls through in
silence.

Now there is one `block_components()` list carrying both the `TypeId` the local
path uses and the name the wire uses, and a `tracing::error!` when a world edit
reaches `classify` with no route. **Two features shipped broken that way and
neither said a word; the third will.**

### Decided, so it is not re-litigated

- **A format is named after its TYPE, not its syntax.** `.blockmesh.ron` collided
  with `Material`'s `ron` and the scan's `find` took the first loader, so a block
  was typed as a material. Now `.block` and `.material`, guarded by
  `no_two_loaders_claim_one_extension`. Documented exception: `glb`/`gltf` are two
  views of ONE file.
- **Adjacency is derived, never stored.** A second copy of one truth drifts, and
  not storing it means the layout can change without breaking a saved level.
- **Collision geometry belongs to an entity**, not to a mesh asset. `Collider.mesh`
  meant "a mesh on disk", and pointing it at a `.block` had two separate walks
  grow the same guard. `MeshKey::Owned(entity)` is what a generated mesh uses.
- **The undo entry carries the whole mesh.** An extrude changes the topology:
  there are faces afterwards that had no before.

### Next

**#1070 — draw the grid the handles already snap to.** `SnapSettings` exists and
nothing paints it, so blocks are aligned blind. 🔴 It carries an unanswered
question: does the grid live on the ground plane, or on the plane the active
handle drags in?

*Answered 2026-09-10: **both**. The world grid at the floor, and a guide at the
selection's height while a handle is dragged — see the entry above.*

---

## 🎯 2026-09-04 — the block tool draws and collides, and five of seven PRs were my own bugs

#946's first slice is half done. A block is an asset, an entity carries it, the
editor draws it and Rapier collides against it. What that took is worth writing
down, because **five of the seven PRs fixed something the previous PR broke**,
and each failure had the same shape: a thing that was absent read exactly like a
thing that was empty.

### The three that cost the most

🔴 **A format is named after its TYPE, not its syntax.** A block was written as
`.blockmesh.ron`. RON is the format, `Material` already claims `ron`, and the
asset scan resolves a loader by extension with a `find` — **first registration
wins**. So a block was typed as a material: the inspector drew it with a base
colour and a roughness slider, and `load_by_guid::<BlockMesh>` found nothing.

Materials are `.material` and blocks are `.block` now, with the invariant that
was missing: `no_two_loaders_claim_one_extension`. It found a second collision on
its first run — `glb` and `gltf`, claimed by both `Mesh` and `MeshletMesh` — kept
as a documented exception, because those are two views of **one** file chosen by
the type the caller asks for. Two loaders reading *different* files is the bug.

The 84 material files moved with their `.meta` sidecars, so every GUID survived
and no scene changed: identity lives in the sidecar, not the path.

🔴 **A generated mesh reaches the GPU through one store, and that store was never
inserted.** `GeneratedMeshes` had exactly two references in the tree, both reads,
both answering `None` forever. Every block built its mesh each frame and dropped
it — and because both sides were written as `if let Some(...)`, **nothing failed**.
An absent store reads identically to "nothing to upload".

The editor builds its `MeshletRenderStage` by hand rather than through
`RenderPlugin`, so it inserts its own beside the stage. `RemoteHostPlugins` is
headless on purpose and correctly has neither — a warning there was an alarm
about a correct absence, which is how alarms become ignorable.

🔴 **A GUID that names a `.block` is not a mesh.** A block's `MeshRenderer.mesh`
and `Collider.mesh` name the GUID of the file the mesh was generated *from*, and
both generic walks resolved it by file and handed the bytes to a glTF parser. For
the collider that was fatal rather than noisy: the parse failed, and
`ColliderMeshCache::fail` is **permanent** — `answered` counts a failure as an
answer — so the body never collided even after the real entry landed a frame
later. Both walks now skip a GUID whose registered type is known and is not a
mesh. An **untyped** GUID still passes, deliberately, and #1078 is the remaining
edge: an asset written by the editor is untyped for a frame.

### What the instrumentation was for

`sync_blocks` only spoke when it failed, and **two processes run it** — the editor
drawing its mirror, the project owning the world. A silence broken only by
failure cannot say which of them built the mesh you are not seeing. It reports
what it published now, and it named both remaining holes on its first run.

### Next, in order, and the order is forced

| | | why there |
|---|---|---|
| **#1079** | Adjacency, derived | pure data, testable without a GPU, blocks the other two |
| **#1080** | Vertex / edge / face selection | needs adjacency to know what an edge is |
| **#1081** | **Extrude** | needs the boundary of a selected face set |

🔴 **Adjacency is DERIVED, never stored.** `BlockMesh` serialises three arrays and
nothing else. A second copy of the same truth drifts the first time an operator
forgets one — and because it is not stored, the structure can change without
breaking a single saved level.

🔴 **A face is a set of corners, and corners are shared.** Dragging a face moves
its corners, and every other face using them follows. It is where a first
implementation goes wrong, and the test that proves it: drag one face of a cube,
assert the opposite face did **not** move while the four side faces changed shape.

🔴 **Extrude's difficulty is several contiguous faces as ONE piece.** A 2×3 patch
pulled up is a wall, not six pillars. An edge used twice within the set is
interior, once is boundary. Direction is the **average** normal — per-face tears a
curved patch into fans.

Reference: `godot-ply`, MIT, winged-edge in SoA, whose `extrude.gd` walks the
boundary loops with zero float tolerances. 🔴 **ProBuilder is Unity Companion
License** — read it, never copy it here.

---

## 🎯 2026-09-03 — the level tool, and why ProBuilder beat CSG

#946 decided CSG over ProBuilder in August, on the grounds that ProBuilder is
*"an order of magnitude more tool"*. Reversed by the owner, and the research
done that same day is why the reversal is right.

### The dependency research, done before any code

| | verdict |
|---|---|
| **`manifold-csg`** | the state of the art, and **C++ behind bindings**. This engine cross-compiles through mingw and had just made its build-requirement list minimal and probed. A C++ dependency taxes **every machine that builds a project**, for a blockout tool |
| **`csgrs` `main`** | depends on eight sibling crates **by path**, none published — a git rev would not build outside its own workspace. And it has left Dimforge for its own `hyper*` stack |
| **`csgrs` 0.20.1** | the only usable version, thirteen months old. Wants `nalgebra ^0.33` against this engine's 0.35, so a **second nalgebra** in a binary that is already 302 MB. Its physics features want rapier 0.24 / parry 0.19 against 0.34 / 0.29 |

⚠️ **crates.io read `csgrs` as abandoned and it is not** — last publish July
2025, last push five days ago, `main` three minor versions ahead. Active
development, stale releases. Check the repository, not the registry.

🔴 The risk is not the version. It is that **an active maintainer's fixes are
out of reach** — which is the inverse of the case the house rule's exception
list covers.

### Why ProBuilder is smaller

**It needs no boolean at all.** Extrude, inset, bevel and bridge are **local
topology edits**: they touch the selected faces and nothing else. Every page of
the graveyard above is about booleans.

The core is three things:

1. **`BlockMesh`, an authoring mesh beside the render one.** `Mesh` is a
   triangle soup with positions duplicated per face, because each face carries
   its own normal — it **cannot** be edited in place. That is exactly why
   ProBuilder keeps `ProBuilderMesh` beside `MeshFilter.mesh`. Generated to a
   `Mesh` on every edit and handed to `build_default_meshlets`, which the
   renderer already eats; the collider comes from the same positions.
2. **Selection at vertex, edge and face.** `viewport_pick` and the handle set
   with snapping already exist.
3. **Extrude.** The one operation that turns a box into a level.

Starting at `BlockMesh` because **its field names are serialised, and a rename
breaks scenes in silence.**

### 🔴 Licence

**ProBuilder is under the Unity Companion License, which is not an open source
licence** — it restricts redistribution outside Unity-dependent projects. This
repository is public and MIT.

Same rule as the UE5 source: reading is legitimate, copying is not. What is
used is the public documentation and algorithms that are textbook — extruding a
face is duplicating its vertices, offsetting along the normal and stitching
quads. If a real implementation is needed, **Godot's CSG is MIT** and can be
adapted; Blender is GPL and cannot.

The owner knows what that buys: a tool of the same shape, written from scratch,
which may differ in details.

## 🎯 2026-09-03 — hot reload of Rust, closed with the record

The whole field, re-checked in one day, so the search is not run again.

### What every implementation says about itself

| | |
|---|---|
| **`subsecond`** (Dioxus, in Bevy 0.17) | *"does not support hot-reloading of structs… if layout or alignment change and new functions are called referencing an old version of the struct, the program will crash."* Only patches the crate holding `main.rs`. Needs Dioxus' CLI and its own linker |
| **`hot-lib-reloader`** 0.8.2, maintained | *"types will have different ids"* after a reload, which breaks a `TypeId`-keyed ECS. Layout changes are *"undefined behavior which will likely result in a crash"* |
| **`dexterous_developer`** | 🔴 **archived 2026-02-08**, Bevy 0.14 |
| **Fyrox**, the one Rust engine shipping CHR | *"a very new and experimental feature"* with *"wildly unsafe functionality"*. Can miss plugin entities → *"crash or memory corruption"*. **Nothing at all about adding or removing a field** |

**Kóoch is in a better position than Fyrox**, by a decision made before any
of this came up: it never holds plugin-typed bytes. Fyrox has to hunt live
objects through reflection and admits it sometimes misses them; here there is
nothing to hunt — everything is `ReflectValue` by name, and the audit found
exactly **one** thing pointing into the `.so`: the library handle.

### 🔴 crABI is not the answer, and this is the part worth keeping

A stable ABI guarantees that a **given declaration** maps to a fixed layout
across compiler versions. It says nothing about a **changed** declaration
being compatible with the old one. Add a field, the layout changes, and code
compiled against the old one is still wrong.

**C has had a stable ABI for forty years and still cannot add a field to a
struct in a shared library.** That is why C libraries use opaque pointers,
versioned symbols and reserved padding.

**Stable ABI ≠ schema evolution.** Two different problems, and only the first
was being watched.

What crABI *would* buy: the `BuildStamp` refusal could go — a plugin built by
a different `rustc` could load. Real, and not what was being waited for.

**And it is stalled.** RFC 3470 open since 2023-08, the implementation PR
since 2022-12, neither merged; human comments on the PR end in **December
2022**. T-lang approved it as an experiment, T-compiler never reviewed it.

### The decision

**Full Rust. No scripting layer.** #733, #121 and #75 closed with their
reasoning in their bodies — including why *nativization* (author in a script,
generate Rust for release) is not the way out: Epic built it for Blueprints
and **removed it in UE 5.0** because the generated code was slower than
handwritten, much harder to debug, and many nodes failed to nativize
all-or-nothing. Worse here, it would mean two different programs, with any
divergence appearing only in the build you cannot iterate on.

⚠️ **Not closed by association**: shader hot reload (#70, #250) and the
declarative UI layer (#280). Those are **data**, not a language, and both are
feasible.

### What to check, and how

```bash
gh api repos/rust-lang/rfcs/pulls/3470 --jq .merged_at   # crABI
```

Four triggers, and if none moved there is no news:

1. **RFC 3470 merges** — and see above, it would fix the stamp, not the field
2. **`subsecond` claims struct support** — today it explicitly does not
3. **Fyrox stops calling its CHR unsafe**
4. Something new appears under Rust + hot-reload

🔴 The one that would actually matter is **2**, and it is not a language
feature: changing a struct across a reload needs access **by name** plus data
migration — which is what this engine already does with `ReflectValue` and
`DynamicComponents`. The missing piece is not in Rust. It is the editor
reading fields from something that is not a compiled binary.
...
Deleting an engine asset breaks **every project on the machine** sharing the
vendored install, not the open one: `assets/meshes` and `assets/materials` ship
with the engine, and a scene referencing one holds no copy.

**What was already right.** The asset browser draws the engine as
`"Engine (read-only)"` with `writable: false`, and `folder_menu` returns early
on it while `leaf_menu` gates Rename / Duplicate / Delete behind the same flag.
No Delete was ever offered on an engine node.

**What was not.** `delete_asset` and `delete_folder` took the path and ran.
The rule existed only in the code that draws the menu — so it covered exactly
the clicks that menu happens to draw, and nothing else. `workspace_for`, in the
same file, has always told the two roots apart; the comment beside it claims
engine assets are read-only and nothing enforced the claim.

🔴 **A guard in the presentation layer is not a guard.** It is a decision about
what to render, and it holds only as long as rendering stays the sole route to
the action. Every other producer — a shortcut, a drag, the wire — arrives at the
handler.

**Project first, engine second, and that ordering is load-bearing.**
`bootstrap::engine_root` walks up for the nearest `assets/`, so an editor built
from a project resolves its engine root to that project. Asking the engine first
would make everything in it undeletable, and the guard would look like it
worked. There is a test named for exactly that.

Project assets are unchanged: permanent, no dialog, no trash, no undo.
...
## Next — the camera rig (#1250), then the graphics queue

**2026-09-21:** #1251, #1252 and #1253 are closed, and roll-a-ball#22 with them. Next is **#1258**, then #1260 and #1261 → #1262.

**2026-09-20:** the layers block is closed (#1218–#1222) and what it was a way into is now written
down: **#1250**, the camera rig audited against Cinemachine and Phantom Camera. Eight pieces, in the
order they are felt — occlusion (#1251), dead zone and soft zone (#1252), lookahead (#1253), the
lens on the vcam (#1254), impulse and shake (#1255), group framing (#1256), a confiner (#1257),
orbit from input and recentering (#1258). **roll-a-ball#22** goes with them: the test project has no
camera input bound at all, and a rig is judged by playing it.

Everything below is the graphics queue, unchanged and still gated by the budget.

## The graphics queue, with the budget as the gate
...
| **#1120** | Japeusa — neural rendering, own design, distilled from DLSSNR | **Pivoted 2026-09-12, still a correctness/design spike, not a committed feature.** Started as a literal DLSSNR port; two findings closed that road. First, `strings` on the local `nvngx_dlssnr.dll` (rel_310_8) confirms the per-frame contract is `Color + MVec + Depth + intensity/reset` — already what `UpscaleInputs` carries for SGSR2/FSR3/DLSS, so no new G-buffer plumbing either way. Second, the kernel payload is **not** the public CUDA fatbin format — `cuobjdump`/`nvdisasm` 13.4 (pulled as standalone RPMs, no system install) both report "does not contain device code", and neither fatbin magic nor `__cudaRegisterFatBinary` appears anywhere in the binary. NVIDIA's real math is unreachable with legitimate tooling; a bit-exact port was never going to be more than a hope. What's left is architecture-as-reference, not weights-as-payload: RTTI class names (`CCSplitSwin16H*Layer`, `CCVit*Layer`, namespace `HNetCpp`) confirm a Swin-window + ViT-global-attention hybrid independent of `iamwavecut/MLX-DLSS`'s own recovered graph; kernel names and a stray training-checkpoint string (`..._Quantize_With_Teacher_...`) confirm distillation + quantization-aware training as the *method*, not the network. New plan: train a much smaller network of our own, sized for plain compute shaders (no cooperative matrix, no FP8 — same ceiling as FSR4 above), distilled against the **real** DLSSNR running through its own public NGX API on a real RTX 3070 (SSH, no weights ever extracted or touched) — the oracle for both training data and correctness, same role a bit-exact diff would have played. Future phase, not started: a modified Proton (OptiScaler-style upscaler-call interception) shipping Japeusa to any Vulkan/DX game, wired into Tatu Launcher with its own config panel |
...
| **#94** (PR #1046) | **A character controller, and the sense pass under it.** A floating capsule: a swept sphere measures the gap and a damped spring holds the body over it, so steps and slopes fall out of one continuous term instead of being special-cased, and the body stays dynamic — it pushes crates and takes hits. Walking is a *goal velocity*, not a push: a floating capsule never touches the floor so it has **no friction at all**, and asking for a velocity makes stopping the same term as starting. Orientation is authored rather than torqued — a torque needs the inertia tensor the backend does not expose, so its gains could only be tuned by feel. The sense pass is the structural claim: `sense::under` classifies what is below as `Ground`, `Step` or `Wall` — a step's riser and a wall give the *same* contact normal, so it is told apart by dropping a second probe past the contact and looking for a ledge — and `sense::beside` sweeps ahead **and to both sides**, because a probe aimed where the character is going never finds the wall it is running along. `Grounded` and `Touching` carry those answers; every mechanic reads them and none of them casts. `WallSlide` and `WallRun` are both here and are different moves, one asking to fall slowly and one asking not to fall |
| **#137** (PR #1041) | **Every rapier collision shape, resolved and bakeable.** Three primitives became the set rapier ships, cut to the ten a person would pick — `Segment`, `Triangle`, `Polyline`, `Voxels`, `VoxelizedMesh` and `Heightfield` keep their discriminants and lose their labels, so old scenes load and the menu stops being a quiz. Mesh-derived shapes reach physics through `ColliderMeshCache`, defined in `kooch_physics` and filled from the facade: a `Guid` in the shape would drag the renderer in behind it. Three measured fixes came out of it — the bridge parses the `.glb` instead of building a meshlet LOD chain it throws away (2 884 → 36 ms on a 76k mesh), the shape resolves once per body build instead of once per frame, and `ConvexPart` carries the faces that stop an already-baked hull being hulled again. Three Inspector buttons bake a hull, convex pieces or a decimated mesh into the project, with a source hash so a stale bake is visible; the engine ships its own under `assets/meshes/collision`. #574 finished with it: hulls and pieces are outlined from the same `resolve` the solver goes through |
| **#47 · #48** | **A floor that is a plane, and a zone that overrules.** `PlaneGravity` is bounded along its normal and unbounded across it — the shape a level floor actually has, which an `AreaGravity` with enormous half-extents was pretending to be, and one-sided because a plane with no thickness that pulled from both sides traps a body inside it. `GravityPriority` is the answer to the case superposition cannot express: a room whose down *replaces* the planet's instead of summing into a slant nobody authored. A higher level suppresses the ones below **in proportion to its own reach**, so the shape of the transition is the shape of that source's `falloff` and a body walking out of the door is not snapped. Sources at equal levels still sum, and a scene with one level takes the old path untouched. `gravity_dominant` rounds out #48's list: up according to the strongest single source, for orientation and never for a force. #45 and #46 closed alongside — verified against the tree, already shipped. The GPU buffer #45 asked for stays unbuilt on purpose: rapier is a CPU solver, so a buffer on the GPU would have to be read back to be used |
