...
| **#1120** | Japeusa — neural rendering experimental, ported from DLSSNR | **Filed 2026-09-12, correctness spike, not a committed feature.** `strings` on the local `nvngx_dlssnr.dll` (rel_310_8) confirms the real per-frame contract is `Color + MVec + Depth + intensity/reset` — no G-buffer wider than what `UpscaleInputs` already carries for SGSR2/FSR3/DLSS. History is the network's own previous output (`dlssnr_prev_output`, RGBA16F), internal state rather than an engine input. Architecture reference is `iamwavecut/MLX-DLSS`'s recovered graph (Apache-2.0 code, BYO weights, never redistributed); 5 presets match `PerfMode`'s existing ladder. Hits the **same `wgpu`/silicon ceiling as FSR4 above** — no cooperative/subgroup matrix, no FP8 on gfx1150 — so a slow result is the same verdict already on record, not a surprise. Phase 1 is an offline harness outside the live render graph: diff Japeusa's output against a real captured DLSSNR run (Color/Depth/MVec in, NVIDIA's own output as the oracle) on both the RTX 3070 and the OneXFly/RX9070XT. Wiring a fourth `UpscaleTechnique` waits on that match |
