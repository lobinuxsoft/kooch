...
✅ **#1118 — fixed (#1146).** Two causes. The element-mode handler returned before `entity_at`, which was right when Face was the only mode and wrong once vertex and edge picked within 12 pixels. Behind it, entity picking tested **bounding boxes**, so a block whose box contains another entity won the click. Blocks are now picked by their triangles, and an entity nearer along the ray beats a vertex or edge it covers. **The rule was not wrong — its premise stopped being true and nothing re-checked it.**

Next: #1106 (cheapest, shortens a real blockout most), #1086 triplanar **before** #1069's
glTF export, and #1087's free list — which #1114 turns from a slow leak into a ceiling
reached in one collapse.
