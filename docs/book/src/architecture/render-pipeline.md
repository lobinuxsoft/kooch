...
1. 🔴 **It is a compile-time feature.** `dlss_wgpu`'s build script links
   ...
   separate package from the loader, and a machine that runs Vulkan
   games has no reason to carry them (`vulkan-headers` on Fedora and
   Arch, `libvulkan-dev` on Debian).
