...
| Editor colour code | `palette.rs` | connected | The icon says what a thing is, the colour says its family, and a state beats its family: prefab blue, scene teal, mesh sand, material violet, shader pink, texture yellow, audio lime, input cyan, block slate; main scene green and dirty amber as states. Used by the Assets rows and the World rows, tested for mutual distance and for legibility on the dark panel. |
