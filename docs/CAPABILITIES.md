...
| `CameraFraming` | `framing.rs` | connected | #1252. Dead and soft zones in screen fractions: the rig follows a tracked point that holds still while the target is in the dead zone, eases after it in the soft zone and is dragged past it. `screen` places the target off centre. Replaces the vcam's position damping. The camera takes on the target's own motion as it crosses the soft zone, so it reaches the hard edge already travelling alongside it (#1288). Drawn over the Game view while the vcam is selected. Part of the rig epic #1250. |
| Collision matrix | `kooch_core/layers.rs`, `kooch_physics/plugin/systems.rs` | connected | #1302. One symmetric table per project — which layers meet which — edited in the layers asset. A `Collider` names the `layer` it is on and the masks rapier needs are derived: memberships is that bit, filter is the matrix's row. A project with no table collides everything with everything, and a collider with authored masks keeps them. |
| Editor colour code | `palette.rs` | connected | The icon says what a thing is, the colour says its family, and a state beats its family: prefab blue, scene teal, mesh sand, material violet, shader pink, texture yellow, audio lime, input cyan, block slate; main scene green and dirty amber as states. A `Family` answers BOTH the icon and the colour, so a row cannot say two things: package, tree, cube, sphere, sparkle, image, speaker, gamepad, shapes, gear, file-code, file-text. Used by the Assets rows and the World rows, tested for mutual distance, for legibility on the dark panel and for one icon per family. |
| Copy / duplicate / paste | `actions/entity_state.rs` | connected | #1292. Duplicate and paste carry the whole subtree: children keep their shape, references into the copy point at the copy, identity is fresh (#1287) and the prefab link is kept and re-rooted onto the copy (#1293). Selecting a parent and its child copies one tree, not two. |
| `CameraLookahead` | `lookahead.rs` | connected | #1253. Frames the target ahead of itself by `lead_time` seconds of its velocity, measured from its own positions (works on targets nothing simulates). Capped by `max_distance`; `ignore_vertical` leaves jumps out; `smoothing_duration` is an exact tween, so stopping returns without a snap. Composes with `CameraFraming`. Part of the rig epic #1250. |
