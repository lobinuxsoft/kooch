...
| Shadows | `kooch_render::shadow` | connected (#476) | Four cascades in one atlas, a depth-only meshlet raster, PCSS in Inti's group. Built on the first frame that finds a `DirectionalLight` with `cast_shadows`, released when nothing wants one — an unlit scene, a headless test and a project with shadows off never allocate the 64 MiB. `tests/csm_shadows.rs` asserts both halves: the floor under a cube darkens, and the floor beside it does not. |
