...
/// Two entities of the same archetype: `Transform`, `Name` and `PerspectiveCamera`, declared in the
/// order the display must *not* keep. The camera is there so a parked `Health` has something to
/// sort *before* — appended blindly it would land after it.
