...
fn flag(fields: &[(String, kooch_ecs::reflect::ReflectValue)], name: &str) -> bool {
...
fn number(fields: &[(String, kooch_ecs::reflect::ReflectValue)], name: &str) -> Option<f32> {
    use kooch_ecs::reflect::ReflectValue;
    ...
    info.components
        ...
        .any(|(name, value)| {
            name == "kind"
                && matches!(
                    value,
                    ReflectValue::U32(kind) if *kind == kooch_physics::components::KIND_DYNAMIC
                )
        })
}
    ...
    info.components
        ...
        .any(|(name, value)| match value {
            ReflectValue::Mat4(matrix) if name == "matrix" => {
                kooch_ecs::hierarchy::GlobalTransform { matrix: *matrix }.has_shear(1e-4)
            }
            _ => false,
        })
}
