...
//!
//! Because the profiler was **adopted, not written**. The flamegraph,
//! the timeline, the frame history, the scope statistics and the
//! play/pause control all come from `puffin_egui`, pinned to the same
//! egui this editor already draws with.
//!
//! What is here is the part no crate supplies: a capture that survives
//! the session, and saying something useful when the feature is off.
    ...
    ui.horizontal(|ui| {
        ...
        if ui.button(label).on_hover_text(hover).clicked() {
            ...
            if starting {
                ...
                // `SNAPSHOT_COUNTDOWN`.
                SNAPSHOT_COUNTDOWN.store(120, std::sync::atomic::Ordering::Relaxed);
