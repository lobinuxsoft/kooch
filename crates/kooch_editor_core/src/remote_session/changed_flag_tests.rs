...
/// A full reply arrives exactly when the project could not honour our revision, so what we hold
/// cannot be trusted to match — even if the reply happens to be empty, which is what a full reply
/// for an empty world looks like.
#[test]
