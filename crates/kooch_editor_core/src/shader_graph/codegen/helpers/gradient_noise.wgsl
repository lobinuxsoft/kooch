...
fn graph_gradient_noise(uv: vec2<f32>, period: vec2<f32>) -> f32 {
    let cell = floor(uv);
    let f = fract(uv);
    let fade = f * f * f * (f * (f * 6.0 - 15.0) + 10.0);
    let a = dot(graph_gradient(graph_wrap(cell, period)), f);
    let b = dot(graph_gradient(graph_wrap(cell + vec2<f32>(1.0, 0.0), period)), f - vec2<f32>(1.0, 0.0));
    let c = dot(graph_gradient(graph_wrap(cell + vec2<f32>(0.0, 1.0), period)), f - vec2<f32>(0.0, 1.0));
    let d = dot(graph_gradient(graph_wrap(cell + vec2<f32>(1.0, 1.0), period)), f - vec2<f32>(1.0, 1.0));
...
fn graph_gradient_fractal(p: vec2<f32>, octaves: f32, roughness: f32, lacunarity: f32, mode: f32, period: vec2<f32>) -> f32 {
    ...
    for (var i = 0; i < count; i = i + 1) {
        let n = graph_gradient_noise(at, tile);
...
fn graph_gradient_warp(p: vec2<f32>, amount: f32, period: vec2<f32>) -> vec2<f32> {
    let push = vec2<f32>(graph_gradient_noise(p + vec2<f32>(5.2, 1.3), period), graph_gradient_noise(p + vec2<f32>(1.7, 9.2), period));
