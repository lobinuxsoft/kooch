...
fn graph_simplex_noise(uv: vec2<f32>, period: vec2<f32>) -> f32 {
...
fn graph_simplex_fractal(p: vec2<f32>, octaves: f32, roughness: f32, lacunarity: f32, mode: f32, period: vec2<f32>) -> f32 {
    ...
    for (var i = 0; i < count; i = i + 1) {
        let n = graph_simplex_noise(at, period);
...
fn graph_simplex_warp(p: vec2<f32>, amount: f32, period: vec2<f32>) -> vec2<f32> {
    let push = vec2<f32>(graph_simplex_noise(p + vec2<f32>(5.2, 1.3), period), graph_simplex_noise(p + vec2<f32>(1.7, 9.2), period));
