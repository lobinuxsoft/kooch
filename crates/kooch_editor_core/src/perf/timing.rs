...
/// PreRender system: measures the wall-clock delta since the last invocation and writes
/// `fps_instant` + `fps_avg` into the perf stats Resource. Skips the first frame (no previous
/// timestamp to diff against) so the average stays clean.
pub(crate) fn frame_timer_system(resources: &mut Resources) {
    ...
    if let Some(prev) = state.last_frame_start {
        ...
        // Guard against pathological deltas (e.g. the editor was suspended / debugger paused).
        // Anything > 1 second of silence resets the moving average so we don't drag a multi-second
        // outlier through the window.
        if frame_ms < 1000.0 {
            ...
            // 🔴 The WORST of the window, not another average. A stutter is the only frame time a
            // person actually feels, and an average over sixty frames is built to hide it: a 30 ms
            // hitch inside a second of 6 ms frames moves the mean by 0.4 ms.
            let worst_ms = state
                .frame_ms_history
                .iter()
                .copied()
...
/// Records the duration of the editor render system into `EditorPerfStats.cpu_frame_ms`. Call sites
/// pass the `Instant` captured at the very top of the render function and invoke this just before
