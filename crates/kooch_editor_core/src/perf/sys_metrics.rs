...
pub(crate) struct SysMetricsState {
    ...
    /// How many refresh cycles have run since boot. sysinfo's `cpu_usage()` returns 0.0 on the very
    /// first refresh because it has no prior sample to delta against — we suppress writing stats
    /// until at least two refreshes have established a real baseline.
    samples_taken: u32,
...
impl Default for SysMetricsState {
    fn default() -> Self {
        // Build the System with the minimum refresh kinds we'll actually query. Skipping disks /
        // networks / components keeps each refresh under a millisecond on Linux.
...
/// PreRender system: refreshes the cached `System` snapshot at most once per `REFRESH_INTERVAL`,
/// then writes the current process's CPU % + RSS into [`EditorPerfStats`].
pub(crate) fn sys_metrics_system(resources: &mut Resources) {
    // 2.082 ms per refresh, measured — a 23% spike on a 9 ms frame, twice a second, for two numbers
    // that may not be on screen (#703). Read before the state is removed so a hidden section costs
    // a resource lookup and nothing else.
    ...
    // Coming back after the section was closed, the previous sample is from whenever it was last
    // open. `cpu_usage()` is a delta against that, so publishing the first reading would report the
    // average CPU over the entire time nobody was looking, as this instant's number.
    ...
    if should_refresh {
        // Refresh global CPU usage FIRST so the per-process delta computed below has a fresh global
        // baseline to compare against. Without this, `process.cpu_usage()` returns 0.0 even after
        // multiple refreshes — empirically observed on Linux during #463 manual validation.
        state
            .system
        ...
        if let Some(proc) = state.system.process(state.pid) {
            ...
            // CPU usage requires a baseline + delta. The first sysinfo refresh records the baseline
            // and `cpu_usage()` returns 0.0; the second refresh produces a real delta.
