...
/// [`PerspectiveCamera::priority`](ome_ecs::perspective_camera::PerspectiveCamera)
...
mod tests {
    ...
    fn rig(follow: u32) -> CameraRig {
        CameraRig {
            follow,
            target: Some(EntityRef::Live(ome_ecs::entity::Entity::new(0, 0))),
            damping: false,
            ..Default::default()
        }
    }
    ...
    fn a_rig_without_a_target_is_inert() {
        ...
        r.target = Some(EntityRef::Live(ome_ecs::entity::Entity::new(0, 0)));
        assert!(!r.is_inert());
    }
