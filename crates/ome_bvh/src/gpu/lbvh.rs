...
/// Dispatch the full Karras LBVH build onto `encoder`. Three passes
/// run sequentially; the AABB propagation pass is dispatched
/// `aabb_iterations(n)` times — one tree level per dispatch.
///
...
///
/// Caller is responsible for ensuring `buffers.ensure_capacity(n)`
/// has been called before this dispatch.
pub fn dispatch_lbvh_build(
    ...
    // -- pass 1: write leaves (sets done[leaf_idx] = 1) --
    ...
    // N==1 is a single leaf with no internals to compute.
    if n <= 1 {
        return;
    }
    ...
    // -- pass 2: karras internal nodes (sets done[i] = 0) --
