...
impl ChunkLodPass {
    ...
    pub fn new(device: &wgpu::Device) -> Self {
        let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("ome_sdf::sparse::chunk_lod::bgl"),
            entries: &BGL_ENTRIES,
        });
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("ome_sdf::sparse::chunk_lod::layout"),
            bind_group_layouts: &[Some(&bgl)],
            immediate_size: 0,
        });
        let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("ome_sdf::sparse::chunk_lod::shader"),
            source: wgpu::ShaderSource::Wgsl(CHUNK_LOD_WGSL.into()),
        });
        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("ome_sdf::sparse::chunk_lod::pipeline"),
            layout: Some(&layout),
            module: &module,
        ...
        let uniform_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("ome_sdf::sparse::chunk_lod::uniform"),
    ...
    pub fn record(
        ...
        let bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("ome_sdf::sparse::chunk_lod::bg"),
            layout: &self.bgl,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
        ...
        let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("ome_sdf::sparse::chunk_lod::pass"),
