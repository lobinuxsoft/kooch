...
use crate::sparse::{LodConfig, ROOT_CELLS};
...
pub(super) fn make_root_indices_buffer(
    ...
    let label = format!("ome_sdf::sparse::root_indices_lod{lod_idx}");
...
pub(super) fn make_subgrid_pool_texture(
    ...
    let label = format!("ome_sdf::sparse::subgrid_pool_lod{lod_idx}");
...
pub(super) fn make_free_list_buffer(
    ...
    let label = format!("ome_sdf::sparse::free_list_lod{lod_idx}");
...
pub(super) fn make_counters_buffer(device: &wgpu::Device, lod_idx: u32) -> wgpu::Buffer {
    let label = format!("ome_sdf::sparse::counters_lod{lod_idx}");
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some(&label),
...
pub(super) fn make_needs_indices_buffer(
    ...
    let label = format!("ome_sdf::sparse::needs_indices_lod{lod_idx}");
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some(&label),
...
pub(super) fn make_needs_count_buffer(
    ...
    let label = format!("ome_sdf::sparse::needs_count_lod{lod_idx}");
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some(&label),
        size: 4,
        usage: wgpu::BufferUsages::STORAGE
...
pub(super) fn make_populate_indirect_args_buffer(
    ...
    let label = format!("ome_sdf::sparse::populate_indirect_args_lod{lod_idx}");
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some(&label),
...
pub(super) fn make_downsample_indirect_args_buffer(
    ...
    let label = format!("ome_sdf::sparse::downsample_indirect_args_c{cascade_idx}");
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some(&label),
        size: DISPATCH_INDIRECT_ARGS_SIZE,
        usage: wgpu::BufferUsages::STORAGE
...
pub(super) fn make_chunk_lod_mask_buffer(device: &wgpu::Device) -> wgpu::Buffer {
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("ome_sdf::sparse::chunk_lod_mask"),
