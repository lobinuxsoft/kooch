...
impl Metrics {
    ...
    pub fn read(grid: &SparseGrid, device: &wgpu::Device, queue: &wgpu::Queue) -> Self {
        let staging = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("ome_sdf::sparse::metrics::readback_staging"),
            size: METRICS_BUFFER_SIZE,
            usage: wgpu::BufferUsages::COPY_DST | wgpu::BufferUsages::MAP_READ,
            mapped_at_creation: false,
        });
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("ome_sdf::sparse::metrics::readback_encoder"),
        });
...
impl MetricsPass {
    ...
    pub fn new(device: &wgpu::Device) -> Self {
        let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("ome_sdf::sparse::metrics::bgl"),
            entries: &BGL_ENTRIES,
        });
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("ome_sdf::sparse::metrics::layout"),
            bind_group_layouts: &[Some(&bgl)],
            immediate_size: 0,
        });
        let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("ome_sdf::sparse::metrics::shader"),
        ...
        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("ome_sdf::sparse::metrics::pipeline"),
            layout: Some(&layout),
            module: &module,
    ...
    pub fn record(
        ...
        let bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("ome_sdf::sparse::metrics::bg"),
            layout: &self.bgl,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
        ...
        let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("ome_sdf::sparse::metrics::pass"),
