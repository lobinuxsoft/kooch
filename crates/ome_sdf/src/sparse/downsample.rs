...
impl DownsamplePass {
    ...
    pub fn new(device: &wgpu::Device) -> Self {
        let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("ome_sdf::sparse::downsample::bgl"),
            entries: &BGL_ENTRIES,
        });
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("ome_sdf::sparse::downsample::layout"),
            bind_group_layouts: &[Some(&bgl)],
            immediate_size: 0,
        });
        let module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("ome_sdf::sparse::downsample::shader"),
        ...
        let pipelines = std::array::from_fn(|cascade_idx| {
            ...
            let label = format!(
                "ome_sdf::sparse::downsample::pipeline_c{cascade_idx}_lod{lod_src}_to_lod{lod_dst}",
            );
    ...
    pub fn record_cascade(
        ...
        let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("ome_sdf::sparse::downsample::pass"),
    ...
    fn create_bg(
        ...
        let label =
            format!("ome_sdf::sparse::downsample::bg_lod{lod_src}_to_lod{lod_dst}");
