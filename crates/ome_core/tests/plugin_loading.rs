...
use ome_core::dynamic::{ComponentBridge, EngineHost, PluginLoader};
use ome_core::resource::Resources;
use ome_core::schedule::Schedule;
use ome_plugin_api::component::ComponentSchema;
...
fn recording_resources() -> (Resources, std::sync::Arc<std::sync::Mutex<Vec<ComponentSchema>>>) {
    ...
    resources.insert(ome_core::dynamic::PluginData::new());
...
fn the_plugins_system_runs_and_its_state_stays_in_the_host() {
    ...
    assert_eq!(
        resources
            .get::<ome_core::dynamic::PluginData>()
            .unwrap()
...
fn a_library_without_the_symbols_is_refused() {
    ...
    assert!(
        matches!(err, ome_core::dynamic::PluginLoadError::LibraryOpen { .. }),
        "got {err}"
    );
