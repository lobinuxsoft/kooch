...
//! - `OME_POWER_PROFILE=battery|plugged|balanced|debug` environment variable
...
/// Detects the profile at startup. Env var `OME_POWER_PROFILE` wins if set.
pub fn detect() -> PowerProfile {
    if let Some(forced) = std::env::var("OME_POWER_PROFILE")
        .ok()
