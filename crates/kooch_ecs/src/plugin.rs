...
fn register_builtin_components(resources: &mut kooch_core::resource::Resources) {
...
impl Plugin for EcsPlugin {
    fn build(&self, app: &mut App) {
        ...
        #[cfg(feature = "dynamic")]
        {
            use kooch_core::dynamic::{ComponentBridge, EntityBridge};
            ...
            app.insert_resource(ComponentBridge::new(|resources, schema| {
                let Some(registry) = resources.get_mut::<DynamicTypeRegistry>() else {
                    return Err(kooch_plugin_api::component::RegisterError::NoRegistry);
                };
