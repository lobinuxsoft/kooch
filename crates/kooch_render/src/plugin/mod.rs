...
fn acquire_and_render(
    ...
    // 🔴 The scene is recorded and submitted BEFORE the swapchain image is asked for, and the order
    ...
    // now a windowed game threw them away. Written into the engine's own metrics rather than kept
    // here, so there is one place that answers "how long did the frame take".
