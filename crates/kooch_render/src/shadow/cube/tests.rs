...
fn faces_of_one_light_are_contiguous() {
    // `textureSampleCompareLevel` on a cube array takes the light index and derives the face from
    // the direction, so the six faces of light `n` have to be layers 6n..6n+6. Interleaving lights
    // would sample another lamp's ceiling.
    for slot in 0..4 {
