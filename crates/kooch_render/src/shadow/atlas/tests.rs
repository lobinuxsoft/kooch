...
/// The quadrant packing these tests used to check is gone with the atlas.
#[test]
...
/// The layer is an index into the texture's layers, so it has to stay inside the count the texture
/// was allocated with. One past the end is not a validation error at sample time — it clamps, and
/// clamping means every cascade past the end silently shares the last one's depths.
#[test]
...
/// Four layers of `size²`, not one texture of `2·size` square. The pixel count is identical — which
/// is the point, the migration off the atlas was not supposed to cost or save memory — so a
/// regression here means somebody changed the layer count without saying so.
#[test]
