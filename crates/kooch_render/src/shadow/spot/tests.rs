...
/// Projecting a point through the record has to put the light's own position at the centre of the
/// map. Getting the view's handedness or its up vector wrong still produces a plausible matrix —
/// and a shadow that is somewhere else in the room.
#[test]
...
/// Reversed-Z with an infinite far plane: near maps to 1 and distance approaches 0. A cascade's
/// orthographic slice is linear either way, so this is the one place in the shadow code where
/// getting the convention backwards is silent — everything renders, inverted.
#[test]
...
/// 🔴 The regression the first smoke found. `texel_world_size` on a spot is an angle per texel, NOT
/// metres: the shader multiplies it by each fragment's own distance to the light.
#[test]
...
/// 🔴 The smoke's second finding: rotating the light moved the lit pool and did NOT move the shadow.
#[test]
