...
/// A silhouette is all a shadow is, so simplification error lands in the outline where nothing
/// hides it. Bevy applies no relaxation to shadow views either — the budget is already in the
/// cascade's own texels, which is the term that makes a distant cascade ask for less detail.
#[test]
