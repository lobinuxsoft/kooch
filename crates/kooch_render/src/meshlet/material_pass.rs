...
/// Default per-material shading body: sampled albedo + tangent-space normal mapping +
/// metal/roughness, shaded by Inti. Compose it with [`compose_material_shader`] before creating the
/// module. Entry points: `vs_fullscreen`, `fs_material`.
pub const MATERIAL_PBR_DEFAULT_BODY: &str = include_str!("../../shaders/material_pbr_default.wgsl");

/// The same shading as [`MATERIAL_PBR_DEFAULT_BODY`], as a compute entry point that owns a 16x16
/// screen tile and reads that tile's froxel light list into workgroup memory once (#824).
...
/// march, then the Inti shading model, then the debug views (or the stub that removes them), then
/// the material-specific body. This stands in for the `#import` a WGSL preprocessor would provide.
