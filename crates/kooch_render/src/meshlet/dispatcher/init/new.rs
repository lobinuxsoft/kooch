...
impl MeshletCull {
    /// Allocates one view's cull buffers, sized for at most `capacity` visible meshlets per frame.
    /// `max_triangles_per_meshlet` controls the fixed `vertex_count` used by the indirect draw —
    /// must match the builder's setting (default `meshlet::DEFAULT_MAX_TRIANGLES`).
    pub fn new(device: &wgpu::Device, capacity: u32, max_triangles_per_meshlet: u32) -> Self {
        ...
        // Per view even though its contents do not depend on the camera.
        ...
        // Initial group_max_err buffer. Sized to a small power of two; the dispatcher grows it
        // geometrically when a scene's group_capacity exceeds this. Storage + COPY_DST so we can
        ...
        // Per-thread reject_reasons buffer (#454.4).
        ...
        // Per-stage cull survivor counters (#454.6). 4 × u32 = 16 B, atomicAdded by the cull shader
        // at each stage tail when `CullParams.debug_active != 0`. Cleared each frame; readback
