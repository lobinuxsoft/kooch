...
impl MeshletCullPipelines {
    ...
    pub fn new(device: &wgpu::Device) -> Self {
        ...
        // Two-binding subset of the pool BGL — keeps the cull entry under the wgpu
        // max_storage_buffers_per_shader_stage limit (8).
        ...
        // Scene-wide cull pipeline (`cs_cull_scene`) — group(0) cull shared with the per-mesh path,
        // group(2) instance buffer + SceneCullParams. group(1) is unused here so the layout array
        // marks it None.
        ...
        // Multi-mesh scene cull (`cs_cull_scene_pool`) — group(0) cull shared with the per-mesh
        // path (the `descriptors` binding at group(0)@1 stays bound but the entry point ignores
        // it), group(1) the GlobalMeshPool, group(2) the scene buffers.
        ...
        // 2-pass cull (#465 + #454.4). Both entries share the same pipeline layout: cull group(0),
        // pool group(1), scene group(2), group_err group(3), debug-reject group(4).
        ...
        // Hi-Z 2-pass cull (#445). Same shader, but pass 1 + pass A bind under an extended layout:
        // cull group(0) gains culled_meshlets + culled_count, scene group(2) gains hi_z_params +
        // pyramid texture.
