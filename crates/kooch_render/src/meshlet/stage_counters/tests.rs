...
fn slot_count_invariant() {
    // The state-machine reasoning in the module doc assumes ≥ 2 slots so a slot is always free
    // while another is in flight. 3 gives a comfortable safety margin against driver-thread
    // callback latency.
