...
// Phase 1.E.3c — multi-mesh scene-wide cull (#446). One dispatch enumerates (instance,
// meshlet_offset) pairs across the entire GlobalMeshPool. Each instance carries `mesh_id` which
// redirects every per-meshlet read through `pool_mesh_descriptors[mesh_id]`.
...
struct PoolMeshDescriptor {
    ...
    // Pool-global base id this mesh's meshlet group_index values were shifted by at registration.
    // Subtract from `m.group_index` to recover the mesh-local group id when computing the
    // per-instance slot in `group_max_err` (#474).
    group_base: u32,
...
// The pool's full bind-group layout exposes vertices / meshlet_vertices / meshlet_triangles at
// bindings 2-4 for the rasterizer + deferred shader.
...
fn run_cull_scene_pool(thread_id: u32) {
    // `meshlets_per_mesh` carries the worst-case (max) meshlet count across every registered mesh;
    // the per-thread bounds check against the instance's actual mesh_descriptor.meshlet_count
