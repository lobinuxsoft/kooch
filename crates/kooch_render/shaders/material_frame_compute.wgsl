...
// `visible_slot + 1` (0 means the sample shaded nothing). The upsample pass compares it against the
// full-resolution vbuf to decide which samples a pixel is allowed to blend, which is what keeps
...
// The block of froxels this tile covers, reduced from its own threads. Held per axis: a linear cell
// index cannot be reduced this way, because two pixels one slice apart differ by 1 in z and by an
// arbitrary amount in the linear index.
...
/// Inti's shading, with the punctual lights taken from workgroup memory instead of the cluster
/// index buffer.
fn shade_from_tile(
...
// The mip level this pixel would sample, computed the way the hardware computes it: the uv
// footprint in texels, log2 of the longer axis.
...
// One colour per whole level, so the frame reads as bands rather than as a gradient — a band that
// moves with the camera is a LOD that works, and a screen of one colour is the fault this view
// exists to show. Blue is level 0, and it warms as the level climbs.
...
fn cs_shade_tile(
    ...
    // the `workgroupBarrier`s its neighbours are still waiting on, which is undefined behaviour and
    // in practice a hang.
    if (lid < 3u) {
    ...
    // 🔴 The representative is chosen from the VISIBILITY BUFFER ALONE — the first covered pixel of
    // the quad, in a fixed order — and never from this dispatch's material.
    var pixel = origin;
    var visibility = 0lu;
    var covered = false;
    let quad = rate * rate;
    ...
    if (clustered && cell_count > 0u && cell_count <= MAX_TILE_CELLS) {
        for (var c = 0u; c < cell_count; c = c + 1u) {
            ...
            // Points and spots are consecutive ranges of the same record, walked as one run — the
            // grid stores them that way precisely so no type test has to exist in the loop.
    ...
    if (mine) {
        ...
        // Only the upsample reads this, and only half rate has one. At full rate the store would be
        // one write per pixel bought nothing — the branch is uniform across the workgroup, so it
