...
// The high-pass above doubles as a grain detector, and the engine feeds this a frame whose contact
// shadows are a dithered ray and whose lights are sampled stochastically. Sharpening that noise is
// sharpening the sampling pattern.
...
// 🔴 Clamped, because `textureLoad` out of bounds returns ZERO in WGSL where `texelFetch` clamps in
// GLSL.
...
fn fs_main(in: Varyings) -> @location(0) vec4<f32> {
    ...
    // The 3x3 cross, which is the whole neighbourhood this reads. b d e f h
    ...
    // RCAS filters with a negative lobe in a cross: output = (w * (b + d + f + h) + e) / (4 * w +
    // 1) and solves for the `w` at which that would clip out of {0, 1}, using the ring's min and
    // max rather than the individual taps so a gradient does not read as an edge.
    ...
    if (DENOISE) {
        ...
        // A high-pass normalised against the local range: near 1 where the centre disagrees with
        // its neighbours as much as they disagree with each other, which is what grain looks like
        // and an edge does not.
        var noise = 0.25 * (b_l + d_l + f_l + h_l) - e_l;
        let range = max(max(b_l, d_l), max(f_l, h_l)) - min(min(b_l, d_l), min(f_l, h_l));
        // ⚠️ The one departure from upstream: the divisor is guarded. A flat neighbourhood has a
        // range of exactly zero and the numerator with it, and 0/0 is a NaN that `saturate` is not
        // required to clean up — one NaN pixel here is a lobe of NaN and a hole in the image.
