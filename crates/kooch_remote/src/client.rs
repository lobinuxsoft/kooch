...
impl RemoteClient {
    ...
    /// Closes one open scene in the project, despawning only its own
    /// entities.
    pub fn close_scene(&self, scene: kooch_core::Guid) -> Result<(), ClientError> {
        self.expect_ok(Method::CloseScene { scene })
    }

    /// Points the project's active slot at an already-open scene.
    pub fn set_active_scene(&self, scene: kooch_core::Guid) -> Result<(), ClientError> {
        self.expect_ok(Method::SetActiveScene { scene })
    }
