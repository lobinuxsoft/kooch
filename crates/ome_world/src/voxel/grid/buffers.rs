...
pub(super) fn make_root_indices_buffer(
    ...
    let label = format!("ome_world::voxel::root_indices_lod{lod_idx}");
...
pub(super) fn make_subgrid_pool_texture(
    ...
    let label = format!("ome_world::voxel::subgrid_pool_lod{lod_idx}");
...
pub(super) fn make_subgrid_pool_sampler(device: &wgpu::Device) -> wgpu::Sampler {
    device.create_sampler(&wgpu::SamplerDescriptor {
        label: Some("ome_world::voxel::subgrid_pool_sampler"),
...
pub(super) fn make_free_list_buffer(
    ...
    let label = format!("ome_world::voxel::free_list_lod{lod_idx}");
...
pub(super) fn make_counters_buffer(device: &wgpu::Device, lod_idx: u32) -> wgpu::Buffer {
    let label = format!("ome_world::voxel::counters_lod{lod_idx}");
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some(&label),
...
pub(super) fn make_needs_indices_buffer(
    ...
    let label = format!("ome_world::voxel::needs_indices_lod{lod_idx}");
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some(&label),
...
pub(super) fn make_needs_count_buffer(
    ...
    let label = format!("ome_world::voxel::needs_count_lod{lod_idx}");
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some(&label),
        size: 4,
        usage: wgpu::BufferUsages::STORAGE
...
pub(super) fn make_populate_indirect_args_buffer(
    ...
    let label = format!("ome_world::voxel::populate_indirect_args_lod{lod_idx}");
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some(&label),
...
pub(super) fn make_downsample_indirect_args_buffer(
    ...
    let label = format!("ome_world::voxel::downsample_indirect_args_c{cascade_idx}");
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some(&label),
        size: DISPATCH_INDIRECT_ARGS_SIZE,
        usage: wgpu::BufferUsages::STORAGE
...
pub(super) fn make_chunk_lod_mask_buffer(device: &wgpu::Device) -> wgpu::Buffer {
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("ome_world::voxel::chunk_lod_mask"),
...
pub(super) fn make_metrics_buffer(device: &wgpu::Device) -> wgpu::Buffer {
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("ome_world::voxel::metrics"),
