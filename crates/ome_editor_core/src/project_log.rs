...
//!
//! The project logs as JSON when the editor hosts it — `OME_LOG_FORMAT`,
...
pub(crate) fn record(buffer: &LogBuffer, line: &str) {
    match parse(line) {
        ...
        // drawn as glyphs.
        None => buffer.push_project(Level::INFO, "cargo", ome_core::strip_ansi(line)),
    }
}
...
fn parse(line: &str) -> Option<(Level, String, String)> {
    let value: ome_remote::serde_json::Value =
        ome_remote::serde_json::from_str(line.trim()).ok()?;
...
mod tests {
    ...
    fn a_json_line_keeps_its_level_target_and_fields() {
        let line = r#"{"timestamp":"2026-07-27T05:00:09Z","level":"WARN","target":"ome_physics","message":"a joint is waiting","entity":7}"#;
        let (level, target, message) = parse(line).expect("parses");

        assert_eq!(level, Level::WARN);
        assert_eq!(target, "ome_physics");
    ...
    fn cargos_output_survives_as_text() {
        assert!(parse("   Compiling ome_physics v0.1.0").is_none());
    }
    ...
    fn text_lines_are_stripped_of_escapes() {
        ...
        record(&buffer, "\u{1b}[32m   Compiling\u{1b}[0m ome_core");

        let entry = &buffer.snapshot()[0];
        assert_eq!(entry.message, "   Compiling ome_core");
