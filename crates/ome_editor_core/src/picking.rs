...
use ome_core::Guid;
use ome_core::aabb::Aabb;
use ome_core::resource::Resources;
use ome_ecs::GlobalTransform;
...
fn local_bounds(resources: &mut Resources, mesh: Guid) -> Option<Aabb> {
    ...
    let handle = handle?;
    let assets = resources.get::<ome_core::assets::Assets<MeshletMesh>>()?;
    let aabb = assets.get(handle)?.aabb;
    // Two `Aabb` types exist — `ome_render`'s carries mesh bounds and
    // `ome_core`'s carries the tested slab intersection. Converting is
